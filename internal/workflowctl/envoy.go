...
func (a app) checkEnvoyCLICommands(root string) error {
	...
	for _, command := range commands {
		first, err := a.commandCaptureWithEnv(root, nil, "go", command.args...)
		if err != nil {
			return fmt.Errorf("run Envoy CLI %s command: %w", command.step, err)
		}
		second, err := a.commandCaptureWithEnv(root, nil, "go", command.args...)
		if err != nil {
			return fmt.Errorf("repeat Envoy CLI %s command: %w", command.step, err)
		}
		if !sameCommandCapture(first, second) {
			return fmt.Errorf("envoy CLI %s command result is not deterministic", command.step)
		}
