...
Named choice/sequence `xs:any`: default-effective or explicit `##other`/`lax`; immutable `WildcardParticle` (`##any`/`strict` or `##other`/`lax`), ranges/locations; nonzero unsupported, `0/0` absent. `anyAttribute`
