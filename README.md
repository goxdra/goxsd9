...
GenerateGo supports global built-in/named Boolean/integer/decimal/nonNegativeInteger/string/token/NMTOKEN and inline string/token/NMTOKEN, plus only default-occurrence direct-choice refs to global Boolean/integer/decimal. Other global inline long-family/identity facts are query-only; mapped local forms unsupported. All precisionDecimal targets are rejected.
