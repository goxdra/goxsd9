...
Direct `xs:any` supports `##any`/strict, `##any`/lax, `##any`/skip, `##other`/lax, `##other`/strict, and positive namespace forms (`##local`, `##targetNamespace`, URI lists). `anyAttribute` defaults `##any`/strict; sequence/choice supports `##any`/lax (omitted/explicit), `##any`/skip (namespace=omitted/explicit; processContents=explicit), `##other`/lax, `##other`/strict; locations preserved.
