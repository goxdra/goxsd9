...
XSD 1.0/1.1 graphs, restrictions, and bounds work; `precisionDecimal` is limited. Global boolean/integer/decimal declarations expose immutable facts; named boolean-restriction types are supported. Direct choice/sequence particles expose ranges; `0/0` is omitted; sequences are query-only. Local element references are immutable particles; validation and generation reject them. `Strict10` reports XSD 1.1 constructs. Malformed input is invalid; digit facets work. `ParseSchema` defaults to `Compatibility`; `ParseSchemaWithPolicy` selects; errors return none. `ValidateInstance` supports text-only global built-in/named boolean and numeric scalars plus supported default-occurrence numeric choices; local boolean particles, sequences/repetition, attributes, broader structures remain unsupported. `GenerateGo` supports deterministic global boolean, integer, decimal, and default-choice Go; boolean facets and local particles remain unsupported.
