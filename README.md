...
XSD 1.0/1.1; `openContent=none` works Compatibility/Strict11, not Strict10; extensions require named empty-content bases; model-less identities/locations. Supported direct non-`0/0` `xs:any` terms queryable; validation/`GenerateGo` reject; only validated omittable `0/0` disappears. `anyAttribute` retains facts/no particle occurrence; broader/unsupported wildcards retain located diagnostics.
Refs retain QName/RefLoc/target/order; groups query, broader reject. Long refs retain bounds; malformed. `nonNegativeInteger` refs query-only; consumers reject. `precisionDecimal` types query in Compatibility/Strict11; Strict10 first; roots validate, inline targets query-only.
...
Local `xs:long`/effective-`negativeInteger`/effective-long in direct/bounded attr-free extensions query-only; consumers reject. Allowlist: direct `xs:integer`; named/inline effective `integer`/`negativeInteger`; built-in/named effective-long; direct `xs:negativeInteger` excluded. Published non-`0/0` inline-long/out-of-slice/list/union/structural return located `FailureUnsupported`/`UnsupportedSchemaSyntaxCode`/`ErrUnsupported`; after syntax/occurrence/input validation, unsupported mapped forms may omit only exact `0/0`; invalid/unresolved/cyclic/wrong-kind/value-constraint/policy failures retain located diagnostics/causes/no `Schema`. Inline Boolean/integer/decimal query-only; only global inline string/token/NMTOKEN elements generate. Global attrs query-only; local/inline schema-boundary reject; `GenerateGo` rejects attrs.
`precisionDecimal`: Strict10 rejects typed `Loc` (`FeatureDatatypeFacets`/`FailureUnsupported`/`XSD3030`/`ErrUnsupported`) before `0/0` omission. Compatibility/Strict11 require mapped `precisionDecimal` owner/child/alternative defaults; non-precision alternatives may be query-only. Default direct choices validate; extension choices query-only/consumer-rejected. Published mapped non-`0/0` direct/extension sequences, non-default direct choices, and rejected inline/anonymous alternatives reject; Compatibility/Strict11 omit valid mapped `0/0` after validation. Anonymous targets consumer-excluded; `GenerateGo` rejects all.
