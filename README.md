...
Local `nonNegativeInteger` declared/named/inline/anonymous non-0/0 forms are rejected at schema construction (no schema); exact local 0/0 is admitted then absent under every policy. Admitted anonymous forms are query-only except global inline string/token/NMTOKEN generation; unsupported forms report type/facet `Loc`.
`precisionDecimal` locals: Compatibility/Strict11 admits built-in/named-effective types only in default choices or bounded attribute-free extensions; consumers reject. It omits `0/0`; Strict10 rejects before omission.
GenerateGo: under all policies, global built-in `nonNegativeInteger` element declarations generate; only these require `abstract=false,nillable=false`, either flag yields `FailureUnsupported`/`GOXSD9029`, nil. Standalone named atomic components generate: built-in fields and named declarations use `StrictInteger`; named-typed fields use generated type. Named final/variety/effective-facet gates unsupported (`FailureUnsupported`/`GOXSD9029`, no output); malformed/stale facts internal (`FailureInternal`/`GOXSD9030`, nil). Inline/anonymous `nonNegativeInteger` is query-only; references remain queryable without target gating, consumers reject.
