...
GenerateGo-only: global built-in/named `nonNegativeInteger` elements generate in Compatibility/Strict10/Strict11 (direct/named/forward/included/imported/chameleon); built-in fields use `StrictInteger`; named-element fields retain `StrictInteger`-backed named types; global inline/anonymous `nonNegativeInteger` retains query facts; `ValidateInstance`/`GenerateGo` reject it; local/inline/choice/sequence/repetition/non-default/list/union/attribute/value-constraint/other-integer-derived consumers unsupported/no-output. Scalar/precisionDecimal exclusions remain.
