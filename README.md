...
XSD 1.0/1.1 graphs/restrictions supported; limited `precisionDecimal`/string facets/`openAttrs`/extensions. Global integer/decimal attrs retain immutable default/fixed facts ([ARCHITECTURE.md](ARCHITECTURE.md)). Named global complex types accept unqualified `mixed="false|0"` (omitted = element-only/unretained/unconsumed) and `defaultAttributesApply="true|false|1|0"` only under XSD 1.1/Compatibility without schema-level `defaultAttributes`; validate/discard it with no public, validator, or generator state. `mixed="true|1"` and schema-level `defaultAttributes`/default-group application unsupported; malformed/contradictory XSD 1.1 forms and anonymous globals invalid; local uses/inline forms, string/boolean/precisionDecimal attributes, and other wildcard/attribute forms/consumers unsupported. Choices/sequences expose exact ranges/attribute-wildcards. `ParseSchema` defaults Compatibility; Strict10 reports the 1.1 mismatch. `ValidateInstance`: named direct integer/decimal sequences, expanded-name order/exact ranges; scalar choices/refs. `GenerateGo`: deterministic scalar/default-choice/default-bounded integer/decimal sequences; repetition/excluded particle/target shapes, strings, unsupported refs. Validation/generation consume no attributes.
