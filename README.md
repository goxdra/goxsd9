...
XSD 1.0/1.1 graphs/restrictions supported; limited `precisionDecimal`/string facets/`openAttrs`/extensions. Global integer/decimal defaults/fixes are immutable. Named global complex types support element-only bodies and `defaultAttributesApply` under XSD 1.1/Compatibility; other mixed forms/default groups unsupported; malformed/contradictory forms and anonymous globals invalid; local/inline uses, attributes, wildcards, and consumers unsupported. Choices/sequences expose exact ranges/attribute-wildcards. Compatibility default; Strict10 reports 1.1 mismatch. `ValidateInstance`: named direct integer/decimal sequences and scalar choices/refs. `GenerateGo`: deterministic scalar/default-choice/default-bounded integer/decimal sequences; other shapes unsupported.
