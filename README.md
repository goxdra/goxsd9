...
XSD 1.0/1.1 graphs and `xs:precisionDecimal` are supported; `Compatibility`/`Strict11` permit precisionDecimal, while `Strict10` reports XSD 1.1 constructs as located unsupported. Malformed input remains invalid. `totalDigits`/`fractionDigits` are supported; broader facets remain unsupported. `ParseSchema` defaults to `Compatibility`; `ParseSchemaWithPolicy` selects a policy. Manifest metadata selects strict policies; `schema/@version` is inert. Errors omit schema. `ValidateInstance` supports built-in/named integer/decimal globals and local integer/decimal choice; precisionDecimal instance validation, attributes, particles, and semantics remain unsupported. `GenerateGo` emits deterministic scalar/choice Go; generation is staged. [Scalar library quickstart](library_example_test.go) is library-only.
