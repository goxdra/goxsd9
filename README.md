...
Global `xs:long` and named atomic-long attributes query under every policy: built-in `xs:long` has intrinsic inclusive bounds `[-9223372036854775808,9223372036854775807]`; named references retain exact effective facets/bounds, locations, provenance, and target IDs; built-ins have no ID. Long defaults/fixed unsupported at value `Loc`; validation/`GenerateGo` reject.
...
`precisionDecimal` locals under Compatibility/Strict11 admit default choices or bounded attribute-free extensions; `0/0` absent, Strict10 rejects first. Extensions stay queryable; validation/GenerateGo reject; all targets GenerateGo-rejected.
