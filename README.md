...
Anonymous Boolean/integer/decimal queryable; mapped anonymous token/NMTOKEN/precisionDecimal schema-unsupported (`0/0` omitted pre-gating); locals consumer-rejected. Typed local precisionDecimal: Compatibility/Strict11 (Strict10 rejects); choice itself and each mapped precisionDecimal alternative require default (1/1) occurrences; unsupported <all> mapping; only non-extension default-occurrence direct choices validate. Non-default precisionDecimal choice/alternative or non-0/0 direct-sequence precisionDecimal ranges schema-unsupported; non-precision non-default alternatives query-only. Token/NMTOKEN queryable; default all-token/all-NMTOKEN choices validate; mixed-family/token/NMTOKEN unsupported. Global inline string/token/NMTOKEN generate; precisionDecimal queryable in Compatibility/Strict11 (Strict10 rejects); anonymous refs consumer-rejected.
