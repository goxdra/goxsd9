...
Direct/supported attribute-free choices/sequences query local built-in/named-effective `xs:unsignedLong`; admitted nonzero forms retain occurrences as query-only facts. Excluded mapped nonzero: inline/anonymous `unsignedLong` and other excluded scalars fail with located `FailureUnsupported` diagnostics; no schema; consumers reject.
Direct `xs:negativeInteger` rejects mapped nonzero; named/anonymous-inline query-only. `precisionDecimal`: Compatibility/Strict11 admits only default direct/bounded-extension choices; Strict10 rejects; permitted `0/0` forms omitted after all gates; mapped nonzero/non-default/sequence/inline/anonymous rejected at direct-choice/extension boundary.
