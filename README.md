...
XSD 1.0/1.1 graphs and restrictions work; limited `precisionDecimal`. Built-in/named scalar and string-enumeration facts are immutable. Bounded `openAttrs` restrictions expose empty content and `##other`/`lax` wildcard facts; validation/generation reject them. Choice/sequence ranges exact; `0/0` omitted; local strings unsupported. Element references are immutable; validation/generation reject them. `Strict10` reports XSD 1.1 constructs. Malformed input is invalid; digit facets work. `ParseSchema` defaults to `Compatibility`; `ParseSchemaWithPolicy` selects policy; errors none. `ValidateInstance` supports text-only built-in/named boolean/numeric scalars plus default-occurrence numeric choices; strings, local particles, sequences/repetition, attributes, and broader structures are unsupported. `GenerateGo` supports deterministic global boolean/integer/decimal, default-choice, and default-bounded direct integer/decimal sequence structs; strings, boolean facets, and local particles are unsupported.
