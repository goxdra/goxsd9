...
Direct `xs:any` supports `##any`/strict, `##any`/lax, `##other`/lax, `##other`/strict; ranges/locations retained. Consumers reject wildcards; other facts/placements unsupported. `anyAttribute` defaults `##any`/strict; named direct sequence/choice supports `##any`/lax (omitted/explicit), `##other`/lax, `##other`/strict; locations preserved.
