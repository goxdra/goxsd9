...
XSD 1.0/1.1 graphs, restrictions, bounds work; limited `precisionDecimal`. Global boolean/integer/decimal, atomic `xs:string` declarations expose immutable type facts; named boolean restrictions supported; named/anonymous string enumeration facts are implemented and immutable. Choice/sequence ranges exact; `0/0` omitted; local string particles unsupported. Local element references are immutable particles; validation/generation reject. `Strict10` reports XSD 1.1 constructs. Malformed input invalid; digit facets work. `ParseSchema` defaults to `Compatibility`; `ParseSchemaWithPolicy` selects; errors none. `ValidateInstance` supports text-only global built-in/named boolean/numeric scalars plus default-occurrence numeric choices; string globals, local boolean/string particles, sequences/repetition, attributes, broader structures unsupported. `GenerateGo` supports deterministic global boolean/integer/decimal, default-choice, default-bounded direct integer/decimal sequence structs; string, boolean facets, local boolean/string particles unsupported.
