...
Mixed XSD 1.0/1.1 graphs, restrictions, ordered bounds, and direct scalar choice/sequence particles work; `precisionDecimal` remains limited. Global boolean, integer, and decimal elements expose exact ranges; effective `0/0` is omitted. Direct local element references are immutable query particles, but validation and code generation reject them. `Strict10` reports XSD 1.1 constructs. Malformed input is invalid; digit facets work. `ParseSchema` defaults to `Compatibility`; `ParseSchemaWithPolicy` selects a policy; errors return none. `ValidateInstance` supports numeric globals and default scalar choices; boolean, sequences, and repetition remain unsupported. `GenerateGo` supports deterministic scalar/default-choice Go; boolean remains unsupported.
