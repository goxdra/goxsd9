...
XSD 1.0/1.1 graphs, restrictions, ordered bounds work; `precisionDecimal` is limited. Global boolean/integer/decimal declarations expose immutable facts; named boolean-restriction types are supported. Direct choice/sequence particles expose exact ranges; effective `0/0` omitted; sequences are query-only. Direct local element references are immutable query particles resolved to one global identity; validation/code generation reject them as unsupported. `Strict10` reports XSD 1.1 constructs. Malformed input is invalid; digit facets work. `ParseSchema` defaults to `Compatibility`; `ParseSchemaWithPolicy` selects; errors return none. `ValidateInstance` supports text-only global built-in/named boolean and numeric scalars plus supported default-occurrence numeric choices; local boolean particles, sequences/repetition, attributes, broader structures remain unsupported. `GenerateGo` supports deterministic scalar/default-choice Go; Boolean remains unsupported.
