...
XSD 1.0/1.1 graphs/restrictions supported; limited `precisionDecimal`/string facets/`openAttrs`/extensions. Named global complexes support element-only bodies/`defaultAttributesApply` under XSD 1.1/Compatibility. Direct global named XSD 1.1 `openContent mode="none"` is inert and accepted under Compatibility/Strict11; Strict10 reports a located XSD 1.1 mismatch. Non-none/wildcard-bearing `openContent`, `defaultOpenContent`, inline/derivation-local forms unsupported; malformed forms invalid. Other groups/shapes, local uses/attributes/wildcards, and consumers remain unsupported. Choices/sequences expose exact ranges/attribute-wildcards. Compatibility default. `ValidateInstance`/`GenerateGo` cover named direct numeric sequences/choices/refs and global scalar generation.
