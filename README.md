...
Global `xs:long`/named attributes query all policies: built-in bounds `[-9223372036854775808,9223372036854775807]`; named refs retain effective facets/bounds, locations, provenance, target IDs; built-ins no ID. Unsupported default/fixed values report `FailureUnsupported`/`UnsupportedSchemaSyntaxCode`/`ErrUnsupported` at value `Loc`; both report `FailureInvalid`/`XSD3010`, fixed primary/default related, no `Schema`. Consumers reject.
