...
Supported named direct sequence/choice types expose attribute-free `anyAttribute` under XSD 1.0/1.1 and Compatibility. Omitted attributes effectively `##any`/`strict`; explicit `##other`/`lax` remains supported. Wildcard element locations retained; omitted default-attribute locations zero. Other wildcard/attribute forms and wildcard-bearing validation/generation remain unsupported. Choice/sequence ranges exact; `0/0` omitted. Element references immutable; validation/generation reject them. Malformed input is invalid; digit facets work. `ParseSchema` defaults to `Compatibility`; `ParseSchemaWithPolicy` selects policy; errors return no schema. `ValidateInstance` supports text-only built-in/named boolean/numeric scalars and default-occurrence numeric choices; strings, local particles, sequences/repetition, attributes, and broader structures unsupported. `GenerateGo` supports deterministic global boolean/integer/decimal, default-choice, and default-bounded direct integer/decimal sequence structs; strings, boolean facets, and local particles unsupported.
