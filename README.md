...
Refs retain QName/RefLoc/target/order; unresolved/wrong-kind/ambiguous/inaccessible invalid with type/base and candidate/target locations. `precisionDecimal` schema/query: built-in/named roots and direct-default local choices validate Compatibility/Strict11; extension choices/global inline/anonymous consumer-rejected; Strict10 rejects. Homogeneous local NMTOKEN sequences validate exact finite/unbounded/above-uint64 occurrences, but GenerateGo rejects; global inline string/token/NMTOKEN elements generate; attributes query-only.
Global attrs query-only admit `xs:long`/`xs:unsignedLong` all policies; built-in refs expose intrinsic inclusive bounds `[-9223372036854775808,9223372036854775807]`/`[0,18446744073709551615]`, named refs exact narrowed/exclusive facets/provenance/ownership. Global element/type facts query-only: built-ins retain intrinsic bounds, named/inline restrictions retain exact effective narrowed/exclusive facets/provenance/ownership; validation/GenerateGo reject.
