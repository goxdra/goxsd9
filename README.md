...
Element/model-group references query-only, retaining QName/RefLoc/TargetID/order without expansion. Nonzero `xs:any` queryable; wildcard consumers reject; broader forms unsupported; `0/0` absent. Global long/unsignedLong facts retain exact bounds/facets/locations/ownership; consumers reject.
All policies: only direct/supported attribute-free extension choices/sequences query local built-in/named-effective `xs:unsignedLong`. Preserve exact inclusive/exclusive bounds; integer enumeration/digit facets; locations/IDs/ownership/provenance; finite/unbounded/above-`uint64` occurrences. Mapped nonzero forms return located `FailureUnsupported` and no schema; inline/anonymous are separate exclusions. Admitted forms are query-only and consumer-rejected.
Direct built-in `xs:negativeInteger` rejects mapped nonzero; named/anonymous-inline query-only. `precisionDecimal` admits default direct/bounded choices only in Compatibility/Strict11; Strict10 first; other/nonzero/inline reject; `0/0` absent.
