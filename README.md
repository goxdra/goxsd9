...
Named direct sequence/choice default-form `xs:any`: immutable `WildcardParticle` (`##any`/`strict`), exact ranges; nonzero unsupported, `0/0` absent. `anyAttribute`
defaults to `##any`/strict or supports explicit `##other`/lax; locations retained.
Direct model-group refs and named global group choices/sequences are query-only; consumers reject expansion. Other groups/shapes, local uses/attributes, broader
`xs:any` unsupported. Consumers support sequences/choices/element refs; scalar generation separate.
