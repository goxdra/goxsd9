...
Element refs retain QName/`RefLoc`/target/order/occurrences; only top-level direct named model-group refs query; nested/local/recursive/broader refs unsupported. Long-family refs retain bounds; built-in/named `nonNegativeInteger` roots validate, long-family roots query-only; malformed refs invalid. Built-in/named/inline `precisionDecimal` query under Compatibility/Strict11; Strict10 rejects all before validation. Only built-in/named roots validate; inline query-only; consumers reject.
