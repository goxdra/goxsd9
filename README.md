...
XSD 1.0/1.1 graphs/restrictions supported; `precisionDecimal`/string-facet/`openAttrs`/extensions limited. Supported typed global integer/decimal attributes: immutable default/fixed facts ([ARCHITECTURE.md](ARCHITECTURE.md)). Supported named global complex bodies accept unqualified `mixed="false"`/`mixed="0"`: omitted element-only (unretained/unconsumed); `mixed="true"`/`mixed="1"` explicitly unsupported; malformed/contradictory XSD 1.1 outer/inner invalid; anonymous globals invalid; inline/unsupported bodies unsupported. Direct choices/sequences expose exact ranges/supported attribute-wildcards; local uses/inline types, string/boolean/precisionDecimal attributes, other wildcard/attribute forms/consumers unsupported. `ParseSchema` defaults Compatibility; policies select; errors: no schema; `Strict10` reports 1.1 constructs; malformed input invalid. `ValidateInstance`: named-global complex types: direct-local integer/decimal sequences, expanded-name order/exact ranges under policies; default scalar choices/refs. `GenerateGo`: deterministic scalar/default-choice/default-bounded integer/decimal sequences; direct-choice repetition/repeated fields, excluded particle/target shapes, local strings/unsupported references unsupported. Validation/generation consume no attributes.
