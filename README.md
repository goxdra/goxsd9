...
`xs:long`/supported named atomic-long attributes join admitted atomics under all policies; built-in bounds `[-9223372036854775808,9223372036854775807]`; named bounds/facets/locations queryable; consumers reject. Each unsupported default/fixed uses `FailureUnsupported`/`UnsupportedSchemaSyntaxCode`/`ErrUnsupported` at value `Loc`; no `Schema`; only default+fixed declarations use `FailureInvalid`/`XSD3010` (fixed primary, default related, no `Schema`).
