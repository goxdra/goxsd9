...
Mixed XSD 1.0/1.1 graphs, restrictions, and bounds work; `precisionDecimal` remains limited. Scalars are immutable. Named complex types expose ordered direct attribute uses, with global refs and prohibited uses, and one simpleContent extension over scalars, including `xs:string`; local `precisionDecimal` follows XSD 1.1. Choice/sequence particles expose ranges; `0/0` omitted and sequences are query-only. Element refs resolve to one global identity; validation/generation reject them. Defaults/fixed, groups, wildcards, and broad derivations remain unsupported. `Strict10` reports XSD 1.1 constructs; malformed input is invalid; errors return none. `ValidateInstance` supports numeric globals and default scalar choices; attributes, boolean, sequences, and repetition unsupported. `GenerateGo` supports deterministic scalar/default-choice Go; attributes and boolean unsupported.
