...
Element refs retain QName/`RefLoc`/target/order/occurrences; only top-level direct named model-group refs query; nested/local/recursive/broader refs unsupported. Long-family refs retain bounds; built-in/named `nonNegativeInteger` roots validate, other long-family roots query-only; malformed refs invalid. Built-in/named/inline `precisionDecimal` query under Compatibility/Strict11; Strict10 rejects all before validation. Built-in/named roots validate; inline query-only; consumers reject.
Local anonymous Boolean/integer/decimal are query-only in direct choice/sequence and bounded extensions; validation/`GenerateGo` reject. Mapped nonzero anonymous string/token/NMTOKEN/`precisionDecimal` and non-string enums are unsupported at type/facet `Loc`; no schema.
