...
XSD 1.0/1.1 graphs/restrictions support limited `precisionDecimal`, string facets, `openAttrs`, and extensions; global integer/decimal attributes retain immutable facts (see [ARCHITECTURE.md](ARCHITECTURE.md)). Direct choices/sequences expose exact ranges/attribute-wildcards and model local exact-string particles with immutable type/facet facts; uses, inline types, token-derived families, attributes, wildcards, and consumers remain unsupported. `ParseSchema` defaults to `Compatibility`; policies select; errors return no schema. `Strict10` reports 1.1 syntax; malformed input invalid. `ValidateInstance` supports named complex types with direct integer/decimal sequences (exact ranges/name order) and scalar choices/refs; modelled local strings remain unsupported. `GenerateGo` emits deterministic scalar/choice/bounded integer/decimal sequences; repetition, excluded shapes, local strings, and references remain unsupported. Validation/generation do not consume attributes.
