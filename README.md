...
XSD 1.0/1.1 graphs/restrictions work; limited `precisionDecimal`; immutable built-in/named scalar/string-enumeration facts. Bounded `openAttrs` restrictions: empty content, `##other`/`lax` wildcard facts; validation/generation reject them. Supported named direct sequence/choice: attribute-free `anyAttribute` under XSD 1.0/1.1/Compatibility; omitted attrs effectively `##any`/`strict`; explicit `##other`/`lax` supported; wildcard element locations retained; omitted default-attribute locations zero. Other wildcard/attribute forms; wildcard-bearing validation/generation unsupported. Choice/sequence ranges exact; `0/0` omitted; local strings unsupported. Element references immutable; validation/generation reject. `Strict10` reports XSD 1.1. Malformed input invalid; digit facets work. `ParseSchema` defaults Compatibility; `ParseSchemaWithPolicy` selects; errors: no schema. `ValidateInstance`: text-only built-in/named boolean/numeric-scalars + default-occurrence numeric choices; unsupported strings/local-particles/sequences/repetition/attributes/broader-structures. `GenerateGo`: deterministic global boolean/integer/decimal, default-choice, default-bounded direct integer/decimal sequence structs; unsupported strings/boolean-facets/local-particles.
