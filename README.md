...
`ParseSchema` exposes immutable components. Callers create `ResolvedSource`; `Resolver` supplies sources. Calls are sequential; contexts/locations stay opaque. Unseen streams close; repeats/cycles close without decoding.

Mixed XSD 1.0/1.1 graphs, declarations, restrictions, digit facets, and optional `xs:precisionDecimal` work. `Compatibility`/`Strict11` permit precisionDecimal; `Strict10` reports XSD 1.1 constructs as located unsupported. Malformed input is invalid. `totalDigits`/`fractionDigits` work; broader facets, chameleon, redefine, override, defaultOpenContent, and assertions are unsupported. `ParseSchema` defaults to `Compatibility`; `ParseSchemaWithPolicy` selects policies, and manifest metadata selects strict policies; `schema/@version` is inert. Errors return no schema. `ValidateInstance` supports text-only built-in/named integer/decimal globals and named complex globals with one direct, non-repeating local integer/decimal choice; precisionDecimal instances, attributes, broader particles, refs, nested particles, wildcards, inline types, and identity constraints remain unsupported. `GenerateGo` emits deterministic scalar/choice Go. The [direct-choice example](direct_choice_example_test.go) uses [fixtures](examples/direct-choice/); run `go test ./... -run '^Example_directChoice$'` to check invalid `XSD2001` at `examples/direct-choice/invalid.xml:2:19` with `xsd11-datatypes#integer`. [Scalar quickstart](library_example_test.go) is library-only.
