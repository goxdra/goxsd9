...
Direct `xs:negativeInteger` rejects mapped nonzero; named/anonymous-inline forms are query-only. Compatibility/Strict11 admit direct precisionDecimal sequences, default choices, and bounded extension choices. Global inline complexes expose anonymous IDs, refs, and uses; direct non-extension precisionDecimal list/union sequence links and global inline restrictions are query-only. Mapped non-default precisionDecimal choices and mapped nonzero local inline precisionDecimal remain unsupported; Strict10 rejects before omission.
