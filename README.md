...
`AttributeUse` facts are ordered/query-only; scalar `simpleContent` bases and uses are queryable, while consumers remain unsupported.
GenerateGo supports global/named `nonNegativeInteger` elements and named types across policies; built-in fields use `StrictInteger`, named fields use generated types. Abstract/nillable and named final/variety/facet gates reject (`FailureUnsupported`/`GOXSD9029`); malformed facts fail internally (`GOXSD9030`). Inline/anonymous forms are query-only.
