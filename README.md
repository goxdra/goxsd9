...
All policies admit local built-in/named-effective `xs:unsignedLong` in direct/supported attribute-free choices/sequences: exact finite, `unbounded`, above-`uint64` occurrences; bounds/facets/IDs/ownership/provenance/locations retained. Admitted: query-only/consumer-rejected; excluded mapped nonzero forms: located `FailureUnsupported`, no schema.
