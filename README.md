...
GenerateGo: under all policies, global built-in and named-typed `nonNegativeInteger` element declarations generate; standalone named atomic components generate. Built-in fields and standalone named declarations use `StrictInteger`; named-typed fields use generated types. Only element declarations require `abstract=false,nillable=false`; either flag yields `FailureUnsupported`/`GOXSD9029`, nil. Named final/variety/effective-facet gates unsupported (`FailureUnsupported`/`GOXSD9029`, no output); malformed/stale facts internal (`FailureInternal`/`GOXSD9030`, nil). Inline/anonymous `nonNegativeInteger` query-only.
