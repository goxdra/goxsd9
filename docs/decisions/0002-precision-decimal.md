...
- Global attributes: Compat/Strict11 admits built-in/named `precisionDecimal` with default/fixed; type-only none. `ValueConstraint()` copies kind, collapsed lexical/source `Loc`, `StrictPrecisionDecimal`; Strict10 rejects at type `Loc`. Inline attrs/consumers unsupported; anonymous `AttributeUse` separate.
- Attribute bodies: Particle-plus-use/direct model-group refs and attribute-only expose ordered `AttributeUse`; simpleContent retains base/type/use `Loc`s and nil particle; bases allow Boolean/string/integer/decimal plus policy-gated `precisionDecimal`. Local/ref allow Boolean/integer/decimal plus policy-gated `precisionDecimal`; refs retain QName/RefLoc/TargetID/use; optional/required effective, prohibited omitted. `form`/`attributeFormDefault` select names; XSD11 local `targetNamespace` must match containing target; absent/mismatch invalid, Strict10 mismatch; chameleon adopts. Anonymous IDs retained; views copied. Unsupported local/inline/global type locations are declaration/`simpleType`/`RefLoc`+target. Unresolved/wrong-kind/ambiguous/inaccessible refs invalid with related locations; no schema. `attributeGroup`/complexContent-extension unsupported.
- Elements: Compatibility/Strict11 admits global built-in/named/inline `precisionDecimal` facts and local built-in/named-effective forms in default-occurrence choices or bounded attr-free extension choices over named empty-content bases. Only mapped precision children/alternatives require default occurrence; non-precision alternatives may remain query-only. Mapped precision non-default choices/sequences, nonzero precision sequences, and mapped nonzero inline/anonymous forms fail `ParseSchema` with `FailureUnsupported`/`XSD3003`/no schema. Exact `0/0` omits after syntax/occurrence checks and applicable reference resolution; Strict10 `XSD3030` rejects first.
- Consumers: Compatibility/Strict11 validates built-in/named roots and non-extension default-occurrence choices. Admitted extension choices and global inline forms remain queryable but consumers reject; inline/anonymous mapping follows the same boundary. `GenerateGo` rejects every precisionDecimal target, and attribute consumers remain unsupported.
