...
- Elements: Compatibility/Strict11 admit built-in/named roots, default choices, and exact sequences. Inline complexes expose IDs, refs, and uses. Extension choices, simpleContent/list/union links, and inline restrictions are query-only. Non-default choices and mapped local anonymous precisionDecimal fail `XSD3003`. `0/0` omits after gates; Strict10 `XSD3030` precedes omission.
- Consumers: Compatibility/Strict11 validates built-in/named roots and default non-extension choices. Extension choices and global inline/anonymous forms remain query-only; their consumers reject. `GenerateGo` rejects precisionDecimal targets and attribute consumers.
