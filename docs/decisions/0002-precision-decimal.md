...
- Schema/query: Compatibility/Strict11 admits global built-in/named/inline `precisionDecimal`; Strict10 rejects by policy. Mapped local built-in/named-effective forms require default direct or bounded attribute-free extension choices; owners/typed alternatives require defaults; inline anonymous, non-default, nonzero direct/extension sequences reject. Strict10 rejects before `0/0`; policies omit it. `AttributeUse`/`AttributeReferenceUse` publish only Boolean/integer/decimal plus policy `precisionDecimal`. Other valid scalar targets fail `FailureUnsupported`/`ErrUnsupported` at schema-syntax `RefLoc`, related declaration, no schema; unresolved/wrong-kind/inaccessible refs invalid. `simpleContent` bases allow Boolean/string/integer/decimal plus policy `precisionDecimal`; Strict10 rejects before admission.
