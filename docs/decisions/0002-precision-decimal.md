...
permit primitive datatypes outside the standard set. Current placement and
consumer boundaries are:
...
- Attribute bodies: Compatibility/Strict11 query-admits local built-in, named-effective, and anonymous `precisionDecimal` `AttributeUse` forms, including grouped extensions, plus references to global `precisionDecimal` attributes. Scalar simpleContent extensions query-admit `precisionDecimal` bases with base/type/use `Loc`s and nil particle. Strict10 rejects these forms at their type-expression `Loc` with `FeatureDatatypeFacets`/`FailureUnsupported`/`XSD3030`/`ErrUnsupported` and no schema. Admitted forms are query-only/consumer-rejected. [Decision 0007](0007-particle-occurrence.md) governs general attribute-body and extension-base rules.
