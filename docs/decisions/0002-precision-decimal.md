...
- Global attributes: Compatibility/Strict11 admits built-in/named `precisionDecimal` with optional default/fixed constraints; type-only declarations have none. `ValueConstraint()` copies kind, collapsed lexical/source `Loc`, and defensive `StrictPrecisionDecimal`. Unsupported/invalid values retain their stable codes and causes; default+fixed uses `XSD3010`. Strict10 rejects at type `Loc`; inline/local forms and consumers remain unsupported, with no schema on error.
- Attribute bodies: Ordered `AttributeUse` facts retain local/ref QName, `RefLoc`, target/use; scalar `simpleContent` facts remain queryable, consumers unsupported.
