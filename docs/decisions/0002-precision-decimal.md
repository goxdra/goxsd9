...
+ Global attributes: Compatibility/Strict11 admit built-in/named `precisionDecimal` with ≤1 default/fixed `AttributeValueConstraint`; type-only none. `ValueConstraint()` copies kind, collapsed lexical/source `Loc`, and defensive `StrictPrecisionDecimal`. Unsupported: `FailureUnsupported`/`UnsupportedSchemaSyntaxCode`/`ErrUnsupported`; invalid: `FailureInvalid`/`XSD3036`; default+fixed: `FailureInvalid`/`XSD3010` (fixed primary/default related). Strict10 rejects type `Loc` before conversion; conflict/type/policy/reference failures precede. Inline/local/consumers unsupported; invalid values retain causes/no schema.
- Elements: Compatibility/Strict11 query built-in/named/inline/anonymous `precisionDecimal`; built-in/named validate; inline/anonymous/`GenerateGo` reject. Mapped owners/typed children/alternatives require defaults. Each owner/term, including inline zero-occurrences, validates syntax/exact-occurrences; resolves base/variety/facets/selected-policy; resolved query-admitted forms map to no public particle; only validated publication-unsupported `FailureUnsupported` diagnostics are omittable at `0/0`. Extension choices query-only; non-`0/0` extensions schema-unsupported. Reject non-`0/0` direct sequences, non-default owners/typed alternatives, published local mapped inline/anonymous non-`0/0`. Strict10 rejects typed/type `Loc` pre-omission, including zero; invalid/unresolved/cyclic/wrong-kind/value-constraint/policy failures retain precedence/locations/causes/no schema.
