...
- Attribute bodies: Particle-plus-use and attribute-only bodies expose ordered local, referenced, and anonymous-inline `AttributeUse` query facts. Local and referenced global targets admit Boolean/integer/decimal plus policy-admitted `precisionDecimal`; Strict10 rejects `precisionDecimal` by policy. `AttributeReferenceUse` retains QName, `RefLoc`, `TargetID`, and effective use. Other valid scalar targets fail schema-syntax `FailureUnsupported`/`ErrUnsupported` at `RefLoc`, relate the target declaration, and return no schema; unresolved, wrong-kind, and inaccessible references remain invalid. A bounded scalar `simpleContent` extension separately admits Boolean/string/integer/decimal bases plus policy-gated `precisionDecimal`, retaining base/type/use locations without a particle; optional/required uses are effective and prohibited uses are omitted. Attribute/simpleContent consumers remain unsupported.
