...
- Elements: Compatibility/Strict11 query global built-in/named/inline/anonymous `precisionDecimal` facts; built-in/named roots validate; inline/anonymous consumers excluded; `GenerateGo` rejects all; mapped owners/typed children/alternatives require defaults. Extension choices query-only; mapped non-`0/0` extension sequences schema-unsupported. Reject mapped non-`0/0` direct sequences, non-default owners/typed alternatives, published local mapped inline/anonymous non-`0/0`; valid mapped `0/0` omits after syntax/occurrence/input validation. Strict10 typed/type `Loc` precedes omission, including zero; failures retain precedence/locations/causes; no schema.
