...
- Elements: Compatibility/Strict11 query built-in/named/inline/anonymous `precisionDecimal`; built-in/named validate; inline/anonymous consumers and `GenerateGo` reject. Mapped owners/typed children/alternatives require defaults. Each owner/term, including inline zero-occurrences, validates syntax/exact occurrences; semantically resolves base/variety/facets/selected policy; only validated publication-unsupported `FailureUnsupported` forms may omit at `0/0`. Extension choices query-only; non-`0/0` extensions schema-unsupported. Reject non-`0/0` direct sequences, non-default owners/typed alternatives, and published local mapped inline/anonymous non-`0/0`. Strict10 rejects typed/type `Loc` before omission, including zero; invalid/unresolved/cyclic/wrong-kind/value-constraint/policy failures retain precedence/locations/causes/no schema.
