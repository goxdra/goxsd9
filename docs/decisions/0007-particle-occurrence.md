...
| --- | --- |
| Query model | Named global complexes expose direct sequence/choice Boolean/integer/decimal/token/NMTOKEN and anonymous Boolean/integer/decimal elements; bounded extensions retain facts/locations. Built-in `xs:long` and supported named effective-long local particles in direct choices/sequences and bounded attr-free extensions are queryable under all policies, preserving written QName, named ID, exact bounds/facets, source/type/facet locations, namespace/nillable/block facts, lexical order; built-ins have no `ComponentID`. Valid mapped non-`0/0` inline long/int/unsignedLong/nonNegativeInteger/nonPositiveInteger/list/union/nested/broader forms are unsupported (`FailureUnsupported`/`ErrUnsupported`); valid mapped `0/0` omits after validation; invalid/unresolved/policy errors retain located diagnostics/no schema. Global inline elements: Boolean/integer/decimal query-only; string/token/NMTOKEN generation-eligible; global attributes query-only; inline-attribute consumers excluded; `GenerateGo` rejects attribute declarations. |
