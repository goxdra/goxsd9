...
range and ordered children into the public schema. The same boundary maps direct `xs:any` `##any`/strict or `##other`/lax to immutable `WildcardParticle` facts with exact locations, ranges, and lexical order; other constraints unsupported. A supported global named
