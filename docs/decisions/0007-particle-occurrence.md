...
| --- | --- |
| Query model | Named global complexes expose direct sequence/choice built-in/named Boolean/integer/decimal/token/NMTOKEN and anonymous Boolean/integer/decimal elements. Bounded attribute-free extensions over named empty-content bases retain facts, identities/locations, particles, and inherited `##other`/lax; model-less retain identity/locations. Local built-in `xs:precisionDecimal` or named effective-`precisionDecimal` types are queryable only in admitted default-occurrence direct or bounded attribute-free extension choices under Compatibility/Strict11; owners and mapped children/alternatives require default occurrences, while nonprecision alternatives may remain query-only. Mapped inline anonymous `precisionDecimal` restrictions are unsupported. Compatibility/Strict11 omit effective `0/0`; Strict10 rejects it before omission. Nonzero precisionDecimal sequences are schema-unsupported. Anonymous facts retain `SimpleTypeID`/`NodeID`/`AnonymousID`, QName ownership, facets, locations, exact occurrences; no `ComponentID`/global-walk ownership. Mapped non-0/0 local anonymous string/token/NMTOKEN/`precisionDecimal` restrictions are schema-unsupported. Local anonymous integer kinds accept only `integer`/`negativeInteger` through named/forward/imported/included/chameleon chains; `long`/`unsignedLong`/`nonNegativeInteger`/`nonPositiveInteger` reject non-0/0 mappings with located `FailureUnsupported`/`ErrUnsupported` at type/facet `Loc` and no schema. Effective `0/0` remains absent after policy admission: Compatibility/Strict11 omit it and Strict10 rejects it first. Supported anonymous Boolean/integer/decimal facets are queryable; mapped non-0/0 non-string anonymous enumeration is unsupported at its facet location with no schema. Global long-family refs retain exact bounds: `long` `[-9223372036854775808, 9223372036854775807]`, `unsignedLong` `[0, 18446744073709551615]`, `negativeInteger` upper `-1`, `nonNegativeInteger` lower `0`, and `nonPositiveInteger` upper `0`; malformed refs are `FailureInvalid`. Global inline string/token/NMTOKEN, Boolean/integer/decimal, long-family, language/NCName/anyURI/ID, and `nonNegativeInteger` retain query facts; global inline/anonymous `nonNegativeInteger` forms are query-only and consumer-rejected; built-in/named `nonNegativeInteger` generates under Compatibility/Strict10/Strict11; inline `precisionDecimal` query-only under Compatibility/Strict11. |
...
| Validation | `ValidateInstance` consumes named global complexes with local built-in/named Boolean-only or integer/decimal sequences in lexical order, honoring exact finite/unbounded/above-`uint64` ranges under Compatibility/Strict10/Strict11; named Boolean is facet-free. Global built-in/named `precisionDecimal` roots validate under Compatibility/Strict11. Global built-in/named `nonNegativeInteger` roots unsupported under Compatibility/Strict10/Strict11: `ValidateInstance` returns located `FailureUnsupported`/`XSD4004`/`ErrUnsupported`; schema-owned bounds/facets unvalidated. Global inline/anonymous `nonNegativeInteger` retains schema/query facts; `ValidateInstance` rejects it (`FailureUnsupported`/`XSD4004`/`ErrUnsupported`). Default non-extension choices validate all-Boolean/token/NMTOKEN alternatives and integer/decimal mixtures; local built-in/named-effective `precisionDecimal` eligible only there. Extensions, anonymous/local consumers, token sequences, mixed/Boolean-numeric choices, direct-choice repetition, and non-default/repeated refs unsupported. Default-occurrence direct-choice refs to global built-in/named Boolean/integer/decimal are supported; sequence/repetition/non-default/nested/recursive/broader/mixed/anonymous-target refs are excluded. |
| `precisionDecimal` | Global built-in/named/inline schema/query admission and built-in/named-root validation are Compatibility/Strict11-only; Strict10 rejects before validation with a policy diagnostic. Global inline is queryable, but anonymous targets are rejected by consumers. Local built-in/named-effective refs require default-occurrence direct choices or bounded attribute-free extension choices; mapped inline anonymous restrictions, non-default choices/alternatives, and nonzero direct sequences reject. Strict10 precedes `0/0` omission; Compatibility/Strict11 omits it. Only non-extension default choices validate; `GenerateGo` rejects every global, local, inline, anonymous, and extension target. |
| Generation | Bounded local Boolean/integer/decimal sequences emit fields; mixed Boolean/numeric, repeated/non-default unsupported; integer/decimal mixtures supported. Under Compatibility/Strict10/Strict11, `GenerateGo` supports global built-in/named atomic `nonNegativeInteger` elements/components in direct/named/forward/include/import/chameleon for ordinary (`abstract=false`, `nillable=false`) declarations; either flag true unsupported (`FailureUnsupported`/`GOXSD9029`, nil). Built-in canonical facts require integer kind/version, fixed `fractionDigits=0`, no `totalDigits`, `minInclusive=0`, no other bounds. Named restrictions retain schema bounds/facets; malformed/stale facts fail closed (`FailureInternal`/`GOXSD9030`, nil). Named forms pass final/variety/effective-facet gates; nonempty final unsupported/no output. Built-in element fields use `StrictInteger`; named `nonNegativeInteger` types are backed by `StrictInteger`, and named-typed elements use the generated named type; bounds/facets are schema-owned. Global inline/anonymous `nonNegativeInteger` is query-only; `GenerateGo`/`ValidateInstance` reject it. Non-0/0 local `nonNegativeInteger`: no schema. Local choices/sequences, repetition/non-default, lists/unions, attributes, other integer-derived, nested/recursive/broader/anonymous targets unsupported/no output. Only non-extension default direct-choice refs to global built-in/named Boolean/integer/decimal are eligible; sequence/repetition/non-default/nested/recursive/broader/anonymous refs rejected. Global/local `precisionDecimal` facts query under Compatibility/Strict11, but `GenerateGo` rejects every global, explicitly typed local (including named-effective), inline, anonymous, extension target; Strict10 rejects global inline by policy. Global inline Boolean/integer/decimal, long-family/identity-only, language/NCName/anyURI/ID facts queryable; anonymous consumers reject. Local built-in/named Boolean/integer/decimal generate only in default all-Boolean/numeric choices and bounded sequences; local anonymous/token/NMTOKEN and repeated/non-default consumers reject. |
