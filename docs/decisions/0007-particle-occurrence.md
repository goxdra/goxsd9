...
| Generation | Default-bounded sequences over local built-in/named integer/decimal or all-Boolean particles emit fields; mixed Boolean/numeric or repeated fields are unsupported, while numeric integer/decimal mixtures remain supported. Non-default direct-sequence occurrences are not emitted as repeated fields. `GenerateGo` also supports default-occurrence all-Boolean or numeric direct choices over local built-in/named Boolean/integer/decimal particles, built-in/named/inherited global Boolean/integer/decimal components, and global built-in, named, inherited, included, imported, and inline string/token/NMTOKEN scalar components. Local anonymous and local token/NMTOKEN particles remain rejected; global inline numeric, Boolean, and `precisionDecimal` declarations remain query-only, while anonymous targets remain consumer-rejected. |
