...
| `precisionDecimal` | Global built-in, named, and inline schema/query admission, plus built-in/named-root validation, applies only under Compatibility/Strict11; Strict10 rejects those schema forms before validation with a located `FeatureDatatypeFacets`/`FailureUnsupported`/`ErrUnsupported` policy diagnostic. Global inline remains queryable under Compatibility/Strict11, but its anonymous target is rejected by validation and generation. Policy admission precedes occurrence omission for mapped local-anonymous forms: Strict10 rejects explicitly typed local `precisionDecimal`, including zero-occurrence cases; Compatibility/Strict11 omit effective `0/0` and admit it in default-occurrence direct choices and bounded attribute-free extension choices. Mapped non-default `precisionDecimal` choice/alternative ranges and mapped non-`0/0` direct-sequence `precisionDecimal` ranges that map to particles are schema-unsupported; nonprecision alternatives may retain non-default query-only ranges. Only non-extension default-occurrence choices validate; extension and anonymous consumers remain unsupported, and `GenerateGo` rejects every global built-in, named, inline, or anonymous `precisionDecimal` target. |
