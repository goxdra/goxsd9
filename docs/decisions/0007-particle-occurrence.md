...
| --- | --- |
| Query model | Named global complexes expose direct sequence/choice built-in/named Boolean/integer/decimal/token/NMTOKEN and anonymous Boolean/integer/decimal elements. Bounded extensions retain facts/locations and inherited wildcards; model-less retain identity. Local effective `precisionDecimal` types are queryable only in admitted default choices or bounded attribute-free extensions under Compatibility/Strict11; inline/nonzero forms reject, `0/0` omits, and Strict10 rejects first. Anonymous facts retain IDs, facets, locations, and occurrences without `ComponentID`/walk ownership. Local integer kinds admit `integer`/`negativeInteger` and built-in/named `unsignedLong`; non-0/0 `xs:int`/`xs:long`/`xs:nonNegativeInteger`/`xs:nonPositiveInteger` and inline unsignedLong forms reject; consumers reject. Local nonNegativeInteger `0/0` is absent under all policies; long-family refs retain bounds and malformed refs invalid. Global inline-element string/token/NMTOKEN declarations retain facts and generate; global inline/anonymous Boolean/integer/decimal/precisionDecimal element/type consumers remain query-only/rejected under Compatibility/Strict11. Global `xs:int`/long-family, identity, and nonNegativeInteger declarations retain facts; nonNegativeInteger refs query, but direct-choice/sequence consumers reject. Global attributes retain admitted type/value facts for query; inline-attribute consumers remain excluded. |
