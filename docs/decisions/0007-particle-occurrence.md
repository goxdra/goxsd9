...
| Generation | Default-bounded local Boolean/integer/decimal sequences emit fields; mixed Boolean/numeric, repeated, non-default unsupported; integer/decimal mixtures supported. Under Compatibility/Strict10/Strict11, `GenerateGo` supports global built-in/named atomic `nonNegativeInteger` elements/components in direct/named/forward/include/import/chameleon, plus global built-in/named Boolean/integer/decimal/string/token/NMTOKEN and inline string/token/NMTOKEN. Built-in canonical facts require integer kind/version, `fractionDigits` exactly 0 and fixed, no `totalDigits`, exactly `minInclusive=0` with no other bounds. Named restrictions may retain schema-owned bounds/facets; malformed/stale named facts fail closed as `FailureInternal`/`GOXSD9030` with nil output. Named forms pass final/variety/effective-facet gates; nonempty final unsupported/no output. Built-in fields use `StrictInteger`; named fields retain backed types; bounds/facets schema-owned, not runtime-validated. Global inline/anonymous `nonNegativeInteger` query-only; `GenerateGo`/`ValidateInstance` reject it. Non-0/0 local `nonNegativeInteger` forms fail schema construction with no schema. Local choices/sequences, repetition/non-default, lists/unions, attributes, other integer-derived, nested/recursive/broader/anonymous targets unsupported/no output. Only non-extension default direct-choice refs to global built-in/named Boolean/integer/decimal supported; sequence/repetition/non-default/nested/recursive/broader/anonymous refs rejected. Global/local `precisionDecimal` facts query under Compatibility/Strict11, but `GenerateGo` rejects every global, explicitly typed local (including named-effective), inline, anonymous, extension target; Strict10 rejects global inline by policy. Global inline Boolean/integer/decimal, long-family/identity-only, language/NCName/anyURI/ID facts queryable; anonymous consumers reject. Local built-in/named Boolean/integer/decimal generate only in default all-Boolean/numeric choices and bounded sequences; local anonymous/token/NMTOKEN and repeated/non-default consumers reject. |
