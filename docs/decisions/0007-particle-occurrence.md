...
| Generation | Default-bounded local Boolean/integer/decimal sequences emit fields; mixed Boolean/numeric, repeated, and non-default fields unsupported; integer/decimal mixtures remain supported. Under Compatibility/Strict10/Strict11, `GenerateGo` supports resolved global built-in/named atomic `nonNegativeInteger` elements/components in direct/named/forward/included/imported/chameleon forms, plus global built-in/named Boolean/integer/decimal/string/token/NMTOKEN and inline string/token/NMTOKEN. Named forms pass final/variety/effective-facet gates; nonempty final controls are unsupported with no `GenerateGo` output. Built-in fields use `StrictInteger`; named fields retain generated type/component ownership backed by `StrictInteger`; bounds are schema-owned, not runtime-validated. Global inline/anonymous `nonNegativeInteger` retains schema/query facts; `GenerateGo`/`ValidateInstance` reject it. Non-0/0 local `nonNegativeInteger` forms are rejected during schema construction with no schema. Choices/sequences, repetition/non-default, lists/unions, attributes/value constraints, other integer-derived, nested/recursive/broader/anonymous targets unsupported; no output. Only non-extension default direct-choice refs to global built-in/named Boolean/integer/decimal supported; sequence/repetition/non-default/nested/recursive/broader/anonymous refs rejected. Global/local `precisionDecimal` facts query under Compatibility/Strict11, but `GenerateGo` rejects every global, explicitly typed local (including named-effective), inline, anonymous, and extension target; Strict10 rejects global inline by policy. Global inline Boolean/integer/decimal, long-family/identity-only, and language/NCName/anyURI/ID facts remain queryable; anonymous consumers reject. Local built-in/named Boolean/integer/decimal generate only in default all-Boolean/numeric choices and bounded sequences; local anonymous, token/NMTOKEN, and repeated/non-default consumers reject. |
