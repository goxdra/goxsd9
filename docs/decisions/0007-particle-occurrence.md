...
| Attribute uses and scalar `simpleContent` | Particle-plus-use and attribute-only bodies expose ordered local, referenced, and anonymous-inline `AttributeUse` facts. `AttributeReferenceUse` retains QName/`RefLoc`/`TargetID`/effective use; references admit Boolean/integer/decimal plus policy-admitted `precisionDecimal`, while other valid scalar targets fail schema-syntax `FailureUnsupported`/`ErrUnsupported` at `RefLoc` with the target declaration related and no schema. Unresolved, wrong-kind, and inaccessible references remain invalid. Bounded scalar `simpleContent` extensions retain base/type/use locations without a particle and admit Boolean/string/integer/decimal plus policy-gated `precisionDecimal`; optional/required uses are effective, prohibited uses omitted, and attribute/simpleContent consumers remain unsupported. |
