...
| --- | --- |
| Query model | Complexes expose sequence/choice Boolean/integer/decimal/token/NMTOKEN and anonymous Boolean/integer/decimal. Bounded attribute-free extension choices/sequences over named empty-content or named `complexContent`-restriction-over-`xs:anyType` bases retain facts/locations/wildcards; model-less identity. All policies admit `integer`, effective named/inline `negativeInteger`, and explicitly typed built-in/supported named-effective `unsignedLong` only in direct choices/sequences or permitted bounded extension choices/sequences. Retain written base QName/base `Loc`, intrinsic built-in or exact named/inline-effective bounds/facets (narrowed/exclusive; Tight max 7), type/facet/use-site `Loc`s, named ID/built-in zero identity, ownership/provenance/exact occurrences; owner/child `0/0` absent. Scalar exclusions (mapped nonzero direct `negativeInteger`; effective `int`/`long`/`nonNegativeInteger`/`nonPositiveInteger`; inline/anonymous `unsignedLong`/unsupported; list/union; narrower) return `FeatureSchemaSyntax`/`FailureUnsupported`/`UnsupportedSchemaSyntaxCode`/`ErrUnsupported` at type/facet/element `Loc`; nested particles return it at nested-particle `Loc`; no `Schema`. negativeInteger/unsignedLong query-only; consumers reject. Resolved element-reference particles and top-level direct model-group refs queryable only after resolution/visibility/wrong-kind/ambiguity/duplicate/occurrence/policy gates; retain QName/RefLoc/TargetID/order/occurrences without target-type gating/expansion; nested/local/recursive/broader refs/groups unsupported/consumer-excluded. Global built-in/named/inline `precisionDecimal` queryable under Compatibility/Strict11; inline/local anonymous consumers reject. Local effective `precisionDecimal` admits only default direct choices/bounded attribute-free extension choices; non-default choices, nonzero sequences, local inline/anonymous reject; Strict10 precedes permitted `0/0` omission; extensions choices-only. Anonymous facts retain IDs/facets/locations/occurrences without `ComponentID`/walk ownership. Local nonNegativeInteger `0/0` absent; global long/unsignedLong/int/identity/nonNegativeInteger facts query-only; refs query, direct consumers reject; malformed refs invalid. Only global inline string/token/NMTOKEN elements generate; other inline/local-attribute consumers excluded. |
...
| `precisionDecimal` | Built-in/named roots validate only under Compatibility/Strict11; inline/anonymous roots query-only/consumer-rejected. Strict10 rejects before validation/`0/0`. Local built-in/named-effective targets validate only in non-extension default direct choices under Compatibility/Strict11; bounded attribute-free extension choices query-only; inline/anonymous targets, non-default choices, and nonzero sequences reject. Admitting policies omit permitted `0/0`. `GenerateGo` rejects every target. |
| Generation | Eligible global forms are named `Boolean`, `integer`, `decimal`, `string`, `token`, `NMTOKEN`, and `nonNegativeInteger` simple-type components; global elements using built-in or those named types; and inline global `string`/`token`/`NMTOKEN` elements. Built-in/named-typed `nonNegativeInteger` elements and standalone named simple-type components generate. Only elements require `abstract=false,nillable=false`; either true is `FailureUnsupported`/`GOXSD9029`, nil. Built-in fields/standalone named declarations use `StrictInteger`; named-typed fields use generated types. Canonical built-in facts require integer kind/version, fixed `fractionDigits=0`, no `totalDigits`, exactly `minInclusive=0`, and no other bounds. Named restrictions retain bounds/facets; final/atomic-restriction-variety/effective-facet gates are `FailureUnsupported`/`GOXSD9029`, no output; malformed/stale facts are `FailureInternal`/`GOXSD9030`, nil. Inline/anonymous `nonNegativeInteger` and `xs:int` elements are query-only; consumers reject. Local non-0/0 `nonNegativeInteger` forms have no schema; `0/0` forms are admitted then absent under every policy. Global `nonNegativeInteger` refs query without target gating; direct-choice/sequence consumers reject nil. Only default direct-choice refs to global built-in/named Boolean/integer/decimal are eligible; broader reject. Local generation is limited to default-occurrence direct Boolean/integer/decimal choices/sequences. Local `unsignedLong` and `precisionDecimal` remain query-only with consumer rejection; token/NMTOKEN, anonymous, repeated, and non-default forms are excluded. Global `long`/`unsignedLong` element/type declarations retain query facts, but validation/`GenerateGo` reject them. Attributes remain query-only; `GenerateGo` rejects every `ComponentKindAttributeDeclaration`. |
| References | Element-reference particles in local content/named groups are queryable immutable facts. Resolution retains expanded QName, `RefLoc`, `TargetID`, lexical order, and exact occurrences without target gating. Global `nonNegativeInteger` refs remain queryable; direct-choice/sequence consumers reject them with located unsupported diagnostics and nil output. Only non-extension default-occurrence direct-choice refs to global built-in/named Boolean/integer/decimal targets are eligible. Sequence, anonymous-target, repetition, nested, recursive, broader, and mixed forms are consumer exclusions; query references retain facts. Model-group refs are a separate top-level direct query boundary: supported named-complex and bounded attribute-free-extension refs retain ordered facts/`TargetID` without expansion; nested/local/recursive/broader refs remain unsupported. |
| Diagnostics and extensions | Syntax, occurrence, element-reference, duplicate, policy gates precede `0/0`; reference/duplicate checks precede zero-owner/term omission. Extension/model-less gates run first; codegen uses extension-primary, validation owner/sequence primary, and omits anonymous locations. After gates, zero owner/term is absent, a zero sequence owner skips child mapping, and zero child is absent. Direct checks use element/particle `Loc`s with related anonymous facts. Model-group refs use group `RefLoc` primary, relate the particle for validation, and retain group/component/reference/target locations for generation. `xs:any`/`anyAttribute` retain ordered facts; broader forms/consumers are unsupported. Reference, malformed-occurrence, policy, and unsupported diagnostics return no partial `Schema`/`GenerateGo`. |
