...
| --- | --- |
| Query model | Named global complexes expose direct sequence/choice local built-in/named Boolean/integer/decimal/token/NMTOKEN and anonymous Boolean/integer/decimal elements. Bounded attribute-free extensions over named empty-content bases retain these facts, base identities/locations, particles, inherited `##other`/lax; model-less retain base identity/locations. Particle-plus-use/attribute-only bodies expose ordered scalar local/ref/inline `AttributeUse`; bounded scalar `simpleContent` retains base/type refs/uses without a particle. Optional/required effective; prohibited omitted; local value/default/fixed/inheritable semantics and attribute consumers unsupported. Local built-in `xs:precisionDecimal` or named effective-facet types are queryable only in admitted default-occurrence direct choices/bounded attribute-free extension choices under Compatibility/Strict11; owners and mapped typed children/alternatives require default occurrences; nonprecision alternatives may query. Inline anonymous `<xs:simpleType><xs:restriction base="xs:precisionDecimal">` restrictions unsupported when mapped. Compatibility/Strict11 omit effective `0/0` for either local form; Strict10 rejects both before omission, including zero. Nonzero precisionDecimal direct/extension sequences schema-unsupported. Anonymous facts retain `SimpleTypeID`/`NodeID`/`AnonymousID`, QName ownership, facets, locations, exact occurrences; no `ComponentID`/global-walk ownership. Mapped non-0/0 local anonymous string/token/NMTOKEN/`precisionDecimal` restrictions schema-unsupported. Local anonymous integer kinds accept only `integer`/`negativeInteger` through named/forward/imported/included/chameleon chains; excluded `long`/`unsignedLong`/`nonNegativeInteger`/`nonPositiveInteger` remain valid but unsupported with located `FailureUnsupported`/`ErrUnsupported` at type/facet `Loc` and no schema. Supported anonymous Boolean/integer/decimal facets are queryable; mapped non-0/0 non-string anonymous enumeration unsupported at its facet location with no schema. Global long-family refs retain exact bounds: `long` `[-9223372036854775808, 9223372036854775807]`, `unsignedLong` `[0, 18446744073709551615]`, `negativeInteger` upper `-1`, `nonNegativeInteger` lower `0`, and `nonPositiveInteger` upper `0`; malformed refs are `FailureInvalid`. Global inline string/token/NMTOKEN, Boolean/integer/decimal, long-family, and language/NCName/anyURI/ID declarations retain query facts; inline `precisionDecimal` does so under Compatibility/Strict11 only. |
