...
| --- | --- |
| Query model | Named global complexes expose direct sequence/choice built-in/named Boolean/integer/decimal/token/NMTOKEN and anonymous Boolean/integer/decimal elements. Bounded extensions retain facts/locations and inherited wildcards; model-less retain identity. Local effective `precisionDecimal` types are queryable only in admitted default choices or bounded attribute-free extensions under Compatibility/Strict11; inline/nonzero forms reject, `0/0` omits, and Strict10 rejects first. Anonymous facts retain IDs, facets, locations, and occurrences without `ComponentID`/walk ownership. Local integer kinds admit only `integer`/`negativeInteger`; non-0/0 `xs:int`/long-family/unsignedLong/nonNegativeInteger/nonPositiveInteger forms reject with `FailureUnsupported`/`ErrUnsupported` and no schema. Local nonNegativeInteger `0/0` is absent under all policies; long-family refs retain bounds and malformed refs invalid. Global inline-element string/token/NMTOKEN and Boolean/integer/decimal declarations, plus global `xs:int`/long-family, identity, and nonNegativeInteger declarations, retain facts; nonNegativeInteger refs query, but direct-choice/sequence consumers reject. Inline/anonymous element/type consumers and inline-element `precisionDecimal` remain query-only under Compatibility/Strict11. Global attributes retain admitted type/value facts for query; inline-attribute consumers remain excluded. |
| Global attribute constraints | All policies admit global attributes with supported named atomic `xs:boolean`, `xs:integer`, `xs:decimal`, `xs:token`, `xs:negativeInteger`, `xs:language`, `xs:NCName`, `xs:anyURI`, `xs:ID`, and `xs:long`. `xs:precisionDecimal` and named restrictions are query-only under Compatibility/Strict11; Strict10 rejects at type `Loc` with `FeatureDatatypeFacets`/`FailureUnsupported`/`XSD3030`/`ErrUnsupported`. Excluded type references report `FailureUnsupported`/`UnsupportedSchemaSyntaxCode`/`ErrUnsupported` at use-site type `Loc`; valid local attributes report it at the local element `Loc`, global inline attributes at the inline `simpleType` `Loc`. Earlier invalid/policy/resolution/reference precedence remains; errors return no schema. Value constraints support only Boolean/integer/decimal/token/precisionDecimal: unsupported individual default/fixed values use `FailureUnsupported`/`UnsupportedSchemaSyntaxCode`/`ErrUnsupported` at value `Loc`; invalid supported values use `FailureInvalid`/`XSD3036` with cause; default+fixed uses `FailureInvalid`/`XSD3010` with fixed primary/default related. Built-in `xs:long` has intrinsic inclusive bounds `[-9223372036854775808,9223372036854775807]`; named refs retain QName/type `Loc`, exact narrowed/exclusive facets, locations, provenance, and target IDs; built-ins have no synthetic `ComponentID`. Precision constraints retain zero or one optional default/fixed `AttributeValueConstraint`; type-only declarations have none, and `ValueConstraint()` copies kind, collapsed lexical/source `Loc`, and exact defensive `StrictPrecisionDecimal` via `PrecisionDecimalValue()` only when present. Attributes are query-only; inline-attribute consumers remain excluded; validation/`GenerateGo` reject; `GenerateGo` rejects every `ComponentKindAttributeDeclaration`; occurrence N/A. |
| Validation | `ValidateInstance` consumes named global complexes with local built-in/named Boolean, integer/decimal, or homogeneous token sequences, honoring exact finite/unbounded/above-`uint64` ranges and token enumeration under all policies. Built-in/named `precisionDecimal` roots validate under Compatibility/Strict11. Built-in/named `xs:int`/long-family/`nonNegativeInteger` roots return located `FailureUnsupported`/`XSD4004`/`ErrUnsupported`; schema bounds/facets are unvalidated, and global inline/anonymous element/type forms are consumer-rejected. Default choices support Boolean/token/NMTOKEN alternatives, integer/decimal mixtures, and eligible local `precisionDecimal`; extensions, anonymous/NMTOKEN sequences, mixed-family, repetition, non-default, nested, recursive, broader, and anonymous-target forms reject. Only non-extension default-occurrence refs to global built-in/named Boolean/integer/decimal are supported. |
