...
| --- | --- |
| Query model | Named complexes expose sequence/choice Boolean/integer/decimal/token/NMTOKEN and anonymous Boolean/integer/decimal elements. Bounded attribute-free extension choices/sequences over named empty-content or named `complexContent`-restriction-over-`xs:anyType` bases retain facts/locations/wildcards; model-less identity. All policies admit `integer`, effective named/inline `negativeInteger`, and explicitly typed built-in/supported named-effective `unsignedLong` only in direct choices/sequences or permitted bounded extension choices/sequences. Retain written base QName/base `Loc`, intrinsic built-in or exact named/inline-effective bounds/facets (narrowed/exclusive; Tight max 7), type/facet/use-site `Loc`s, named ID/built-in zero identity, ownership/provenance/exact occurrences; owner/child `0/0` absent. Mapped nonzero direct `negativeInteger` and excluded `int`/`long`/`nonNegativeInteger`/`nonPositiveInteger`, inline/anonymous, list/union, narrower/nested forms return located `FeatureSchemaSyntax`/`FailureUnsupported`/`UnsupportedSchemaSyntaxCode`/`ErrUnsupported` at type/facet/element `Loc`, no `Schema`. Effective negativeInteger/unsignedLong query-only; consumers `FailureUnsupported`. Resolved element-reference particles and top-level direct model-group refs queryable only after resolution/visibility/wrong-kind/ambiguity/duplicate/occurrence/policy gates; retain QName/RefLoc/TargetID/order/occurrences without target-type gating/expansion; nested/local/recursive/broader refs/groups unsupported/consumer-excluded. Global built-in/named/inline `precisionDecimal` facts queryable under Compatibility/Strict11; inline/local anonymous consumers reject. Local effective `precisionDecimal` admits only default direct choices/bounded attribute-free extension choices; non-default choices, nonzero sequences, local inline/anonymous reject; Strict10 precedes permitted `0/0` omission; extensions choices-only. Anonymous facts retain IDs/facets/locations/occurrences without `ComponentID`/walk ownership. Local nonNegativeInteger `0/0` absent; global long/unsignedLong/int/identity/nonNegativeInteger facts query-only; refs query, direct consumers reject; malformed refs invalid. Only global inline string/token/NMTOKEN elements generate; other inline/local-attribute consumers excluded. |
