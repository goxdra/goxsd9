...
3. Component construction applies the shared exact `0/0` absence rule to
   ...
   `0/0`, including supported query-only named effective-long and forms
   schema-unsupported at publication. Non-`0/0` unsupported mapped forms retain
   located diagnostics; invalid/resolution/cyclic/wrong-kind/value-constraint/
   policy failures retain diagnostics/causes/locations and no `Schema`. It derives
   `mapsToParticle` from exact `0/0`; it does not store an `absent` flag alongside
   the two bounds.
...
| --- | --- |
| Query model | Compatibility, Strict10, and Strict11 admit supported direct local `xs:long` and named effective-long particles in direct choices, direct sequences, and bounded attribute-free extensions for query only. Integer-derived locals (not exhaustive): direct `xs:integer`; named/inline effective `integer`/`negativeInteger`; direct `xs:negativeInteger` excluded. Other Boolean/decimal, token/NMTOKEN, and admitted anonymous forms queryable. Choices/sequences/bounded attr-free extensions retain QName/bounds/facets/occurrences/nillable/block/`Loc`s/order/provenance. Built-in `xs:long`: use-site type/variety locations, intrinsic bounds/zero-bound-facet locations, no synthetic `ComponentID`; named effective-long: `TypeID`/`ComponentID`, declaration/restriction-facet provenance; admitted anonymous forms use `NodeID`/`AnonymousID`. Both are query-only; validation/`GenerateGo` reject both with located unsupported diagnostics/no output. Effective-`negativeInteger` query-only; consumers reject. After syntax/occurrence/input validation, any validated omittable mapped form may be absent only at exact `0/0`, including named effective-long; non-`0/0` unsupported mapped forms return located `FailureUnsupported`/`UnsupportedSchemaSyntaxCode`/`ErrUnsupported` at `Loc`/no schema. Nested/recursive/broader excluded; invalid/resolution/cyclic/wrong-kind/value-constraint/policy failures retain diagnostics/causes/no schema. |
...
| `precisionDecimal` | Compatibility/Strict11 admit global built-in/named/inline/anonymous element/type facts to queries; built-in/named roots validate, inline/anonymous consumers exclude, `GenerateGo` rejects all. Validate syntax/occurrence/input first; Strict10 rejects typed/type `Loc` before consumers/`0/0`; malformed occurrence is `FailureInvalid` at `Loc`. Under those policies, mapped owners/typed children/alternatives require default occurrences. Extension choices query-only; mapped non-`0/0` extension sequences schema-unsupported. Reject mapped non-`0/0` direct sequences, non-default owners/typed alternatives, and published local mapped inline/anonymous non-`0/0`; valid mapped `0/0` omits after validation, including schema-unsupported forms. Failures retain diagnostics/causes/no schema. |
