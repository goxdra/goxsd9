...
| Global attribute constraints | N/A: global attribute declarations have no particle occurrence. Their type/value admission, diagnostics, and query-only consumer boundaries are documented in [Architecture](../../ARCHITECTURE.md#schema-model) and [package documentation](../../doc.go). |
...
Local attributes, attribute groups, and validation/`GenerateGo` consumers remain separate boundaries. Omitted direct
