...
| Global attribute constraints | Global attribute allow-list remains. Compatibility/Strict10/Strict11 admit queryable global `xs:long` and named atomic-long attributes: built-in `xs:long` has intrinsic inclusive bounds `[-9223372036854775808,9223372036854775807]`; named refs retain exact effective integer facets/bounds (narrowed/exclusive included), facet/variety locations, provenance, target IDs; built-ins lack synthetic ID. All three reject `xs:int`, `xs:unsignedLong`, `xs:nonNegativeInteger`, `xs:nonPositiveInteger`, narrower built-ins, list/union, and local/inline/anonymous forms at type/facet `Loc` with no schema. Long default/fixed alone: `FailureUnsupported`/`UnsupportedSchemaSyntaxCode`/`ErrUnsupported` at value `Loc`; default+fixed: `FailureInvalid`/`XSD3010`, fixed primary/default related, no schema. [precisionDecimal follows Decision 0002](0002-precision-decimal.md). Validation/`GenerateGo` reject; occurrence N/A for global attributes. |
| Validation | `ValidateInstance` consumes named global complexes with local built-in/named Boolean-only or integer/decimal sequences, honoring exact finite/unbounded/above-`uint64` ranges under Compatibility/Strict10/Strict11; homogeneous local built-in/named token sequences honor occurrences and validate token enumeration. Built-in/named `precisionDecimal` roots validate under Compatibility/Strict11. Built-in/named `xs:int`/long-family/`nonNegativeInteger` roots unsupported under all policies: returns located `FailureUnsupported`/`XSD4004`/`ErrUnsupported`; schema-owned bounds/facets unvalidated. Global inline/anonymous `xs:int`/long-family/`nonNegativeInteger` retain schema/query facts; consumers reject them (`FailureUnsupported`/`XSD4004`/`ErrUnsupported`). Default choices validate Boolean/token/NMTOKEN alternatives and integer/decimal mixtures; local built-in/named-effective `precisionDecimal` eligible only there. Extensions and anonymous consumers, NMTOKEN sequences, mixed-family choices/sequences, choice repetition, and non-default/repeated refs unsupported. Default direct-choice refs to built-in/named Boolean/integer/decimal are supported; sequence/repetition/non-default/nested/recursive/broader/mixed/anonymous-target refs are excluded. |
| `precisionDecimal` | Global built-in/named/inline facts are queryable, and built-in/named roots validate, only under Compatibility/Strict11; Strict10 rejects before validation. Global inline is queryable but anonymous consumers reject. Local built-in/named-effective refs require default direct choices or bounded attribute-free extension choices; mapped inline anonymous, non-default, and nonzero-sequence forms reject. Strict10 precedes `0/0` omission; Compatibility/Strict11 omits it. Only non-extension default choices validate; `GenerateGo` rejects every target. |
...
broader wildcard/attribute forms unsupported. Excluded long-family attribute shapes
(`int`, `unsignedLong`, `nonNegativeInteger`, `nonPositiveInteger`, and narrower
built-ins), list/union, and local/inline/anonymous forms remain unsupported;
validation/`GenerateGo` reject admitted global long attributes. The matrix retains
the supported global attribute allow-list.
