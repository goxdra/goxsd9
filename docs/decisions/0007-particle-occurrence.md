...
| --- | --- |
| Query model | Named global complexes expose sequence/choice built-in/named Boolean/integer/decimal/token/NMTOKEN and anonymous Boolean/integer/decimal elements. Bounded attribute-free extension choices over named empty-content bases or named `complexContent` restrictions of `xs:anyType` retain facts/locations/inherited wildcards; model-less retain identity. Direct scalar admission (all policies): `integer`, effective named/inline `negativeInteger`, and explicitly typed built-in/supported named-effective `unsignedLong` only in direct choices/sequences or permitted bounded extension choices. Retain QName, built-in/named ownership, exact unsignedLong `[0,18446744073709551615]` bounds/facets, type/facet/use-site `Loc`s, IDs, provenance, occurrences; owner/child `0/0` absent. Mapped nonzero direct `xs:negativeInteger` and mapped scalar exclusions (`int`/`long`/`nonNegativeInteger`/`nonPositiveInteger`, inline/anonymous, list/union, narrower/nested) return located `FeatureSchemaSyntax`/`FailureUnsupported`/`UnsupportedSchemaSyntaxCode`/`ErrUnsupported` at type/facet/element `Loc`, no `Schema`. Admitted effective negativeInteger/unsignedLong query-only; `ValidateInstance`/`GenerateGo` return `FailureUnsupported`. Element refs and top-level model-group refs are separate query-only boundaries retaining QName/RefLoc/TargetID/order/occurrences without gating/expansion. Effective `precisionDecimal` is queryable only in admitted default choices/bounded attribute-free extension choices under Compatibility/Strict11; inline/nonzero rejects, `0/0` omits, Strict10 first. Anonymous facts retain IDs/facets/locations/occurrences without `ComponentID`/walk ownership. Local nonNegativeInteger `0/0` absent; long-family refs retain bounds; malformed refs invalid. Inline string/token/NMTOKEN generate; other inline scalar consumers are query-only/rejected. Global int/long/identity/nonNegativeInteger facts queryable; refs query, direct consumers reject. |
