...
| Global attribute constraints | Global `xs:long` and named atomic-long attributes are queryable with QNames, bounds, facet/variety/provenance, and target IDs; built-ins have no ID. Long defaults/fixed unsupported at value `Loc`. Excluded forms unsupported without schema. `precisionDecimal` unchanged: Compatibility/Strict11 query; Strict10 rejects at type `Loc`; invalid values return none. Validation/`GenerateGo` reject. |
...
Long attributes are queryable under all policies; defaults/fixed unsupported at
value `Loc`; other attribute forms and consumers remain unsupported.
