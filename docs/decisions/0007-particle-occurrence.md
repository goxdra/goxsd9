...
3. Component construction applies the shared exact `0/0` absence rule to
   sequence, choice, and child occurrences after syntax, occurrence, and input
   validation. Any validated omittable mapped form may be absent only at exact
   `0/0`, including supported query-only named effective-long. Non-`0/0` unsupported
   mapped forms retain located unsupported diagnostics; invalid, unresolved, cyclic,
   wrong-kind, value-constraint, and policy failures retain diagnostics/causes/
   locations and no `Schema`. It derives `mapsToParticle` from exact `0/0`; it does
...
| --- | --- |
| Query model | Integer-derived local allowlist (not exhaustive): direct `xs:integer`; named/inline effective `integer`/`negativeInteger`; direct built-in `xs:long` and named effective-long; direct `xs:negativeInteger` excluded. Other Boolean/decimal, token/NMTOKEN, and admitted anonymous forms queryable. Direct choices/sequences/bounded attr-free extensions retain QName/bounds/facets/occurrences/nillable/block/`Loc`s/order/provenance. Built-in `xs:long`: use-site type/variety locations, intrinsic bounds/zero-bound-facet locations, no synthetic `ComponentID`; named effective-long: `TypeID`/`ComponentID`, declaration/restriction-facet provenance; admitted anonymous forms use `NodeID`/`AnonymousID`. Direct built-in `xs:long`/named effective-long locals query-only; validation/`GenerateGo` reject both with located unsupported diagnostics/no output. Effective-`negativeInteger` locals query-only; consumers reject. After syntax/occurrence/input validation, any validated omittable mapped form may be absent only at exact `0/0`, including named effective-long; non-`0/0` unsupported mapped forms return located `FailureUnsupported`/`UnsupportedSchemaSyntaxCode`/`ErrUnsupported` at `Loc`/no schema. Nested/recursive/broader excluded; invalid/resolution/cyclic/wrong-kind/value-constraint/policy failures retain diagnostics/causes/no schema. |
...
| `precisionDecimal` | Global built-in/named/inline/anonymous element/type facts are query-only in Compatibility/Strict11; Strict10 rejects at typed/type `Loc` (`FeatureDatatypeFacets`/`FailureUnsupported`/`XSD3030`/`ErrUnsupported`) before consumer validation/`0/0`; global policy precedes consumers. Validate syntax/occurrence/input first; malformed occurrence `FailureInvalid` at `Loc`. Compatibility/Strict11 require default mapped-owner and typed precisionDecimal child/alternative occurrences; non-precision alternatives may retain non-default query-only ranges. Default direct choices validate; extension choices query-only; mapped non-`0/0` extension sequences schema-unsupported. Reject mapped non-`0/0` direct sequences, non-default owners/typed `precisionDecimal` alternatives, and local mapped inline/anonymous particles; valid mapped `0/0` absent only after validation. `GenerateGo` rejects all targets; failures retain diagnostics/causes/no schema. |
