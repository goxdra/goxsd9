...
`all` mapping, and broader wildcard forms remain unsupported. Unsupported
attribute uses include value constraints (including default/fixed), local/inline
forms, attribute validation, and `GenerateGo`; global built-in/named
`nonNegativeInteger` attribute type facts remain query-only. Exact occurrences
have no fixed resource limit; later phases must bound input/materialization.
