...
| Generation | Default-bounded local Boolean/integer/decimal sequences emit fields; mixed Boolean/numeric, repeated, and non-default fields unsupported; integer/decimal mixtures remain supported. Under Compatibility/Strict10/Strict11, `GenerateGo` supports resolved-graph global built-in/named atomic `nonNegativeInteger` elements/components in direct, named, forward, included, imported, and chameleon forms, plus global built-in/named Boolean/integer/decimal/string/token/NMTOKEN and inline string/token/NMTOKEN. Built-in element fields use runtime `StrictInteger`; named element fields retain generated named type/component ownership, whose underlying scalar value uses `StrictInteger`; bounds are schema-owned, not runtime-validated. Global inline/anonymous `nonNegativeInteger` retains schema/query facts; `GenerateGo` rejects it (unsupported/no output). Local/inline `nonNegativeInteger`, choices/sequences, repetition/non-default, lists/unions, attributes/value constraints, other integer-derived types, nested/recursive/broader/anonymous targets unsupported; no output. Only non-extension default-occurrence direct-choice refs to global built-in/named Boolean/integer/decimal are supported; sequence/repetition/non-default/nested/recursive/broader/anonymous refs rejected. Global/local `precisionDecimal` facts remain queryable under Compatibility/Strict11, but every global, explicitly typed local (including named-effective), inline, anonymous, and extension target is rejected by `GenerateGo`; Strict10 rejects global inline with its policy diagnostic. Global inline Boolean/integer/decimal, long-family/identity-only, and language/NCName/anyURI/ID facts remain queryable; their anonymous consumers reject. Local built-in/named Boolean/integer/decimal generate only in default all-Boolean/numeric choices and default-bounded sequences; local anonymous, token/NMTOKEN, and repeated/non-default consumers reject. |
