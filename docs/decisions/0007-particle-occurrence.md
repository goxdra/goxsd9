...
| --- | --- |
| Query model | Complexes expose sequence/choice Boolean/integer/decimal/token/NMTOKEN and anonymous Boolean/integer/decimal/negativeInteger. Bounded attribute-free extensions over named empty-content/complexContent-over-`xs:anyType` bases retain facts/locations/wildcards; model-less identity. All policies admit `integer`, effective named/inline `negativeInteger`, and explicitly typed built-in/supported named-effective `unsignedLong` only in direct/permitted bounded extension choices/sequences. Retain written base QName/base `Loc`, exact intrinsic/named/inline-effective bounds/facets (narrowed/exclusive; Tight max 7), type/facet/use-site `Loc`s, named ID/built-in-zero identity, ownership/provenance/exact occurrences; owner/child `0/0` absent. Mapped-non-`0/0` exclusions (direct `negativeInteger`; effective `int`/`long`/`nonNegativeInteger`/`nonPositiveInteger`; inline/anonymous `unsignedLong`/unsupported; list/union; narrower) return `FeatureSchemaSyntax`/`FailureUnsupported`/`UnsupportedSchemaSyntaxCode`/`ErrUnsupported` at type/facet/element `Loc`; nested particles at nested-particle `Loc`; no `Schema`. Ordinary `0/0`, including excluded long-family/`unsignedLong`, is admitted after gates then absent. negativeInteger/unsignedLong query-only; consumers reject. Element/top-level model-group refs are queryable after applicable resolution/visibility/wrong-kind/ambiguity/occurrence/policy gates; duplicate checks named-group-only, not ordinary direct choices/sequences; retain QName/RefLoc/TargetID/order/occurrences; no target gating/expansion; nested/local/recursive/broader refs/groups unsupported/consumer-excluded. Global built-in/named/inline `precisionDecimal` queryable in Compatibility/Strict11; inline/local anonymous consumers reject. Local effective `precisionDecimal` admits only default direct choices/bounded attribute-free extension choices; non-default choices, nonzero sequences, local inline/anonymous reject; Strict10 precedes permitted `0/0` omission; extensions choices-only. Anonymous facts retain IDs/facets/locations/occurrences without `ComponentID`/walk ownership. Local nonNegativeInteger `0/0` absent; global long/unsignedLong/int/identity/nonNegativeInteger facts query-only; refs query, direct consumers reject; malformed refs invalid. Only global inline string/token/NMTOKEN elements generate; other inline/local-attribute consumers excluded. |
