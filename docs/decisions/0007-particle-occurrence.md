...
| --- | --- |
| Query model | Allowlist: direct `xs:integer`; named/inline effective `integer`/`negativeInteger` where admitted; built-in/named effective-long; direct `xs:negativeInteger` excluded. Direct choice/sequence/bounded attr-free extensions retain QName/bounds/facets/occurrences/nillable/block/`Loc`s/order/provenance. Built-in `xs:long`: intrinsic bounds/locations, without synthetic `ComponentID`; named effective-long: named `TypeID`/`ComponentID`/provenance; `NodeID`/`AnonymousID` only for admitted anonymous forms. Effective-`negativeInteger`/effective-long locals query-only; consumers reject located unsupported diagnostics/no output. After syntax/occurrence/input validation, unsupported mapped forms may omit only exact `0/0`; published non-`0/0` exclusions (inline-long/int/unsignedLong/nonNegativeInteger/nonPositiveInteger/list/union) return `FailureUnsupported`/`UnsupportedSchemaSyntaxCode`/`ErrUnsupported` at `Loc`/no schema. Nested/recursive/broader excluded. Only validated omittable mapped `0/0` disappears; invalid/resolution/cyclic/wrong-kind/value-constraint/policy errors retain located diagnostics/causes/no schema. |
...
| `precisionDecimal` | Global built-in/named/inline element/type facts query in Compatibility/Strict11; roots validate; Strict10 rejects before validation/`0/0`; inline consumers reject. Mapped local owners/typed precisionDecimal children/alternatives require defaults; non-precision alternatives may retain non-default query-only ranges. Non-extension default direct choices validate; extension choices query-only/consumer-rejected. Published mapped rejections: non-`0/0` direct/extension sequences, non-default direct precisionDecimal choices/alternatives, rejected mapped inline/anonymous forms; schema construction rejects them. Under Compatibility/Strict11, valid omittable mapped `0/0` is absent only after validation, including extension sequences; invalid/unresolved/policy errors retain located diagnostics/no schema. `GenerateGo` rejects every target. |
