...
| --- | --- |
| Query model | Named global complexes expose direct sequence/choice local built-in or named Boolean/integer/decimal/token/NMTOKEN elements and local anonymous atomic Boolean/integer/decimal restrictions; bounded attribute-free extensions over named empty bases retain the same facts, base identities/locations, particles, and inherited `##other`/lax facts; model-less extensions retain only base identity/locations, with no particle/occurrence/content. Anonymous facts retain `SimpleTypeID`/`NodeID`, no `ComponentID`/global-walk ownership, written base QName, resolved named ownership, facets, locations, and exact occurrences. Mapped non-0/0 local anonymous string/token/NMTOKEN/`precisionDecimal` restrictions are schema-unsupported; effective `0/0` omission follows the applicable type-policy admission, while non-default supported ranges remain query-only. Effective local anonymous integer kind accepts exactly `integer`/`negativeInteger` for mapped non-0/0 particles through named/forward/imported/included/chameleon chains; `long`/`unsignedLong`/`nonNegativeInteger`/`nonPositiveInteger` are valid but unsupported there, producing located `FailureUnsupported`/`ErrUnsupported` at the type/facet `Loc` with no schema. Supported anonymous Boolean/integer/decimal facets are queryable; mapped non-0/0 non-string anonymous enumeration is unsupported at its facet location with no schema. Global built-in/named long-family references retain exact integer bounds: `long` `[-9223372036854775808, 9223372036854775807]`, `unsignedLong` `[0, 18446744073709551615]`, `negativeInteger` upper `-1`, `nonNegativeInteger` lower `0`, and `nonPositiveInteger` upper `0`; malformed references are `FailureInvalid`. Global inline string/token/NMTOKEN, Boolean/integer/decimal, long-family, and language/NCName/anyURI/ID atomic declarations retain query facts; inline `precisionDecimal` does so under Compatibility/Strict11 only. |
| Validation | `ValidateInstance` consumes named global homogeneous Boolean/numeric sequences in lexical declaration order and honors exact finite, unbounded, and above-`uint64` outer and child ranges under Compatibility, Strict10, and Strict11. Only non-extension default-occurrence homogeneous all-token or all-NMTOKEN choices validate. Supported non-precisionDecimal non-`0/0` choice ranges remain queryable and effective total ranges are not calculated; integer/decimal mixtures remain supported; precisionDecimal ranges follow the schema-unsupported limits below. Token/NMTOKEN sequences, Boolean/numeric, token/non-token, and NMTOKEN/non-NMTOKEN choices, direct-choice repetition, and local anonymous inline atomic consumers remain unsupported; those consumers return located `FailureUnsupported`/`ErrUnsupported` diagnostics. Non-extension default-occurrence direct-choice references to global built-in/named Boolean/integer/decimal targets are supported; sequence, repetition/non-default, nested/recursive/broader, mixed, and anonymous-target reference consumers are not. |
| `precisionDecimal` | Policy admission precedes occurrence omission: Strict10 rejects explicitly typed local `precisionDecimal` with a located `FeatureDatatypeFacets` `FailureUnsupported`/`ErrUnsupported` policy-mismatch diagnostic, including zero-occurrence cases. Compatibility/Strict11 omit effective `0/0`; under those policies, explicitly typed local `precisionDecimal` is supported only in direct choices whose choice and each mapped `precisionDecimal` alternative use default (1/1) occurrences. Only mapped non-default `precisionDecimal` choice/alternative ranges and non-`0/0` direct-sequence `precisionDecimal` ranges that map to particles are schema-unsupported; non-precision alternatives may retain non-default query-only ranges. Local anonymous `precisionDecimal` is schema-unsupported when mapped, not query-only. Global inline `precisionDecimal` remains a valid schema/query target under Compatibility and Strict11, is rejected by Strict10 at its type location, and its anonymous target is rejected by validation and generation. |
| Generation | Default-bounded sequences over local built-in/named integer/decimal or all-Boolean particles emit fields; mixed Boolean/numeric or repeated fields are unsupported, while numeric integer/decimal mixtures remain supported. Non-default direct-sequence occurrences are not emitted as repeated fields. `GenerateGo` supports global built-in/named/inherited/included/imported Boolean/integer/decimal and string/token/NMTOKEN scalar components, plus global inline string/token/NMTOKEN scalar components. It also supports only non-extension default-occurrence direct-choice references to global built-in/named Boolean/integer/decimal targets; sequences, repetition/non-default occurrences, nested/recursive/broader references, and anonymous targets remain rejected. Global `precisionDecimal` (built-in, named, or inline) is queryable but rejected by `GenerateGo`; Strict10 rejects global inline `precisionDecimal` with the located policy diagnostic. Global inline Boolean/integer/decimal, long/unsignedLong/negativeInteger/nonNegativeInteger/nonPositiveInteger, and language/NCName/anyURI/ID declarations retain query facts but their anonymous validation and generation consumers are rejected. Local built-in/named Boolean/integer/decimal particles generate only in default-occurrence all-Boolean/numeric direct choices and default-bounded direct sequences; local anonymous and token/NMTOKEN consumers and repeated/non-default particles remain rejected. |
