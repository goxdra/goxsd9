...
| Validation | `ValidateInstance` consumes global complexes with local built-in/named Boolean, integer/decimal, or homogeneous token/NMTOKEN sequences, honoring exact finite/unbounded/above-`uint64` ranges and token/NMTOKEN enumeration. Built-in/named `precisionDecimal` roots validate under Compatibility/Strict11. Built-in/named `xs:int`/long-family/`nonNegativeInteger` roots return located `FailureUnsupported`/`XSD4004`/`ErrUnsupported`; schema bounds/facets are unvalidated, and unsupported global inline/anonymous element/type forms are consumer-rejected. Default choices support Boolean/token/NMTOKEN alternatives, integer/decimal mixtures, and eligible local `precisionDecimal`; extensions, anonymous, mixed-family, repetition, non-default, nested, recursive, broader, and anonymous-target forms reject. Homogeneous local NMTOKEN sequences retain exact finite/unbounded/above-`uint64` occurrences under all policies. Only non-extension default-occurrence refs to global built-in/named Boolean/integer/decimal are supported. |
