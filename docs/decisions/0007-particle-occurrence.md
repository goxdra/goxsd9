...
| --- | --- |
| Query model | Allowlist: direct `xs:integer`; named/inline effective `integer`/`negativeInteger`; built-in/named effective-long; direct `xs:negativeInteger` excluded. Direct choices/sequences/bounded attr-free extensions retain QName/bounds/facets/occurrences/nillable/block/`Loc`s/order/provenance. Built-in `xs:long`: use-site type/variety locations, intrinsic bounds, zero bound-facet locations, no synthetic `ComponentID`; named effective-long: named `TypeID`/`ComponentID`, declaration/restriction-facet provenance; `NodeID`/`AnonymousID` only for admitted anonymous forms. Effective-`negativeInteger`/effective-long locals query-only; consumers reject located unsupported diagnostics/no output. After syntax/occurrence/input validation, only exact `0/0` unsupported mapped forms may omit; published non-`0/0` exclusions return located `FailureUnsupported`/`UnsupportedSchemaSyntaxCode`/`ErrUnsupported` at `Loc`/no schema. Nested/recursive/broader excluded; failures retain diagnostics/causes/no schema. |
...
| `precisionDecimal` | Global element/type facts query in Compatibility/Strict11; roots validate; global policy precedes consumer validation. Construction: syntax/occurrence/input validation first; malformed occurrence: `FailureInvalid` at own `Loc`; Strict10 rejects typed `Loc` before `0/0` omission. Compatibility/Strict11 require default occurrences for mapped owners and typed `precisionDecimal` children/alternatives; non-precision alternatives may retain non-default query-only ranges. Default direct choices validate; extensions query-only. Reject mapped non-`0/0` direct/extension sequences, non-default mapped owners or typed `precisionDecimal` alternatives, inline/anonymous forms; valid mapped `0/0` absent only after validation. `GenerateGo` rejects targets; failures retain diagnostics/causes/no schema. |
