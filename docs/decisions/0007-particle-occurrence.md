...
range and ordered children into the public schema. The boundary maps direct `xs:any` `##any`/strict, `##any`/lax, `##any`/skip (`processContents`), `##other`/lax, or `##other`/strict to immutable `WildcardParticle` facts with exact locations, ranges, and lexical order; other constraints unsupported. A supported named
