...
| --- | --- |
| Query model | Local allowlist: direct `xs:integer`; named/inline effective `integer`/`negativeInteger` (excluding direct built-in `xs:negativeInteger`); built-in or supported named effective-long. Supported direct choice/sequence and bounded attribute-free extension query facts preserve QName, `ComponentID`/`NodeID`/`AnonymousID` ownership (built-ins none), bounds/facets, occurrences, nillable/block, `Loc`s, lexical order, and named/forward/import/include/chameleon provenance; effective-long is query-only and consumers reject. Valid non-`0/0` scalar exclusions—inline `long`, `int`, `unsignedLong`, `nonNegativeInteger`, `nonPositiveInteger`, list/union—return `FailureUnsupported`/`UnsupportedSchemaSyntaxCode`/`ErrUnsupported` at `Loc`; no schema. Nested/recursive/broader shapes are separate structural exclusions. Only validated omittable mapped `0/0` disappears; invalid, unresolved, cyclic, wrong-kind, value-constraint, and policy errors retain diagnostics/causes/locations/no schema. |
...
| `precisionDecimal` | Global built-in/named/inline element/type facts query under Compatibility/Strict11; roots validate, Strict10 rejects before validation, inline consumers reject. Local built-in/named-effective forms require default occurrences for the owner and each mapped typed child/alternative in choices/bounded attr-free extensions; mapped non-`0/0` inline/anonymous and non-default/nonzero sequences are unsupported. Strict10 rejects before `0/0` omission; Compatibility/Strict11 omits valid `0/0` after validation; invalid/unresolved/policy errors retain located diagnostics/no schema. Only non-extension default choices validate; `GenerateGo` rejects all precisionDecimal targets. |
...
| Diagnostics and extensions | Direct checks use element/particle locations and may relate anonymous locations. Complex-content/model-less extension gates run first; codegen uses extension-primary facts, validation uses owner or sequence-instance primary, and no validation diagnostic adds an anonymous location. Direct/extension model-group refs use group `RefLoc` primary, relate the group particle for validation, and retain group/component/reference/target locations for generation. Supported `xs:any`/`anyAttribute` forms retain ordered namespace/process facts; effective `0/0` follows occurrence rules, broader forms and wildcard consumers remain unsupported; gates return no `GenerateGo` output. |
