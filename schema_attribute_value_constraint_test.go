...
func TestSchemaBridgeGlobalAttributeValueConstraintDiagnostics(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "unsupported nonNegativeInteger default",
		...
		{
			name:    "unsupported nonNegativeInteger fixed",
			root:    `<xs:schema xmlns:xs="` + testXSDNamespace + `"><xs:attribute name="value" type="xs:nonNegativeInteger" fixed="0"/></xs:schema>`,
			policy:  Strict11,
			class:   FailureUnsupported,
			code:    UnsupportedSchemaSyntaxCode,
			specRef: schemaAttributeValueConstraintXSD11SpecRef,
