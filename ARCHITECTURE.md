...
Complexes: `IsAbstract()` (non-inherited); named types: effective `final`; `Final()`: canonical extension-then-restriction; `FinalLoc()`: supplying location; XSD 1.0/1.1/Compatibility; `final=extension` or `#all` rejects extension.
