...
Attributes retain value constraints. Global `xs:long`/named atomic-long attributes are admitted under all policies with bounds `[-9223372036854775808,9223372036854775807]`, facet/variety locations, and named IDs; built-ins have no synthetic ID. Long defaults/fixed remain unsupported at value `Loc`. Other integer-derived, list/union, and inline/anonymous forms are unsupported with no schema; validation/`GenerateGo` reject.
