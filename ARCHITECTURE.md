...
Global built-in/named/inline `precisionDecimal` schema/query admission: Compatibility/Strict11 only; built-in/named roots validate, inline targets do not. Strict10 rejects all three pre-validation with located `FeatureDatatypeFacets`/`FailureUnsupported`/`ErrUnsupported`. Local-anonymous precision is separate: policy precedes `0/0`; Strict10 rejects typed forms including zero, while Compatibility/Strict11 omit zero and admit default direct/bounded attr-free extension choices. Mapped non-default choices/non-`0/0` direct sequences reject; nonprecision alternatives query-only. Only non-extension defaults validate; extensions/anonymous consumers and all `GenerateGo` targets reject.
...
`ValidateInstance` supports global built-in/named scalar roots (`Boolean`/`token`/`NMTOKEN`/`integer`/`decimal`/`precisionDecimal`) and named complexes. Local built-in/named Boolean/integer/decimal sequences honor exact finite, unbounded, and above-`uint64` ranges. Non-extension default direct choices use local built-in/named Boolean, token, NMTOKEN, integer, decimal, or `precisionDecimal`; all-Boolean, all-token, all-NMTOKEN, and integer/decimal mixtures validate. Mixed Boolean/numeric or token/NMTOKEN choices, repetition/non-default choices, extensions (including schema-admitted precisionDecimal), and anonymous consumers remain unsupported.
