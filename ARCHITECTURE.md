...
Primitive: `DeclaredType`; bounded attribute-free local choice/sequence/extensions retain anonymous Boolean/integer/decimal refs and explicit local `precisionDecimal`, IDs, QName/facets/locations/exact occurrences/bounds, and zero `ComponentID`/global-walk ownership. Effective `0/0` is absent after policy admission: Compatibility/Strict11 omit either form; Strict10 rejects both before omission, including zero.
...
Named complexes: omitted/`false`/`0` accepted; `true`/`1` rejected. Malformed/contradictory XSD 1.1 input is `FailureInvalid`; well-formed specification-valid behavior outside supported slice is `FailureUnsupported`; diagnostics retain stable code, primary `Loc`, cause, `SpecRef`. Attrs: `IsInheritable` (Compatibility/Strict11 accept; Strict10 mismatches; untyped/inline unsupported); `defaultAttributesApply` validated/discarded (Strict10 mismatches). `xpathDefaultNamespace` inert; XPath unsupported.
...
Global `precisionDecimal` is queryable only under Compatibility/Strict11; built-in/named roots validate, inline does not; Strict10 rejects all three before validation. Local forms under that policy differ: explicitly typed local `type="xs:precisionDecimal"` is admitted only in default direct choices and bounded attribute-free extension choices; inline anonymous `<xs:simpleType><xs:restriction base="xs:precisionDecimal">` mapped restrictions are unsupported. Policy precedes `0/0`: Compatibility/Strict11 omit zero; Strict10 rejects both before omission, including zero. Non-default choices/nonzero direct sequences reject; other alternatives query-only. Typed defaults validate; extension/anonymous consumers and `GenerateGo` reject it.
...
`ValidateInstance` supports global built-in/named scalar roots (`Boolean`/`token`/`NMTOKEN`/`integer`/`decimal`/`precisionDecimal`) and named complexes. Local Boolean/integer/decimal sequences honor exact finite, unbounded, and above-`uint64` ranges. Non-extension default choices use local built-in/named Boolean, token, NMTOKEN, integer, decimal, or explicitly typed `precisionDecimal`; homogeneous Boolean/token/NMTOKEN and integer/decimal mixtures validate. Mixed Boolean/numeric or token/NMTOKEN choices, repetition/non-default choices, extensions, and anonymous consumers remain unsupported.
...
Inline Boolean/integer/decimal/long-family/identity-only declarations retain query facts; anonymous consumers reject. Local built-in/named Boolean/integer/decimal default all-Boolean/numeric choices and default-bounded sequences generate; anonymous/token/NMTOKEN and repeated/non-default consumers reject.
