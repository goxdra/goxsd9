...
Supported global attributes retain default/fixed `AttributeValueConstraint`; `ValueConstraint()` returns kind, lexical, `Loc`, and `StrictPrecisionDecimal` facts under Compatibility/Strict11. Global `xs:int`/long-family attribute declarations/value constraints remain unsupported: located `FailureUnsupported`/`ErrUnsupported`, no `Schema`. Strict10 rejects; local/inline attributes, validation, and `GenerateGo` reject. Supported global element/simple-type refs to `xs:int`/long-family retain bounds; malformed refs invalid.
