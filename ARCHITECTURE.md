...
Built-in `xs:long`/`xs:nonNegativeInteger`/`xs:nonPositiveInteger` refs retain exact effective integer bounds; global built-in/facet-free named Boolean/integer/decimal/token constraints are supported; local/inline attributes, broader/non-atomic forms, validation, and Go generation remain unsupported.
