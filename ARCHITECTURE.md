...
Groups/extensions retain IDs/locations; model-less extensions: empty bases/nil particles/inherited `##other`/lax. `anyAttribute`: defaults `##any`/strict; named sequence/choice owners support omitted/explicit `##any`/lax, `##other`/lax, and `##other`/strict; locations preserved. `xs:any`: supports `##any`/strict, `##any`/lax, `##other`/lax, `##other`/strict; ranges; `0/0` absent. Consumers reject wildcards; other placements unsupported. `openContent=none` supports globals/extensions in Compatibility/Strict11; Strict10 mismatch. Unsupported derivation; malformed=invalid.
