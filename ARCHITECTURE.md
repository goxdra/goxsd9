...
Built-in `xs:nonNegativeInteger`/`xs:nonPositiveInteger` refs retain effective zero lower/upper bounds; global built-in/facet-free named Boolean/integer/decimal/token constraints are supported; local/inline attributes, broader/non-atomic forms, validation, and Go generation remain unsupported.
