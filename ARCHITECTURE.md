...
Named complexes accept omitted/`false`/`0`, reject `true`/`1`; malformed XSD 1.1 is invalid,
...
`precisionDecimal` refs require default-occurrence direct or bounded extension
choices; nonzero sequences, inline/anonymous targets, and non-default choices
remain unsupported. Homogeneous local built-in/supported
...
Compatibility/Strict11 omits it. Built-in/named roots and direct default choices
validate under Compatibility/Strict11; extension choices are query-only and
`GenerateGo` rejects every target; refs remain queryable;
