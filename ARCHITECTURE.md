...
Generation: `GenerateGo` supports global built-in/named atomic `nonNegativeInteger` elements/components. Named forms require final/variety/facet gates; nonempty final unsupported with no `GenerateGo` output. Built-in fields use `StrictInteger`; named fields retain types backed by it; bounds are schema-owned. For `nonNegativeInteger`, non-0/0 local schema construction returns no schema; global inline/anonymous query-only forms are consumer-rejected. Local choices/sequences, repetition/non-default, lists/unions, attributes, integer-derived, nested/recursive/broader/anonymous targets unsupported/no output. Built-in/named Boolean/integer/decimal/string/token/NMTOKEN and inline string/token/NMTOKEN generate; only default direct-choice Boolean/integer/decimal refs are eligible.
