...
Generation: `GenerateGo` supports global built-in/named atomic `nonNegativeInteger` elements/components. Named forms require final/variety/facet gates; nonempty final unsupported with no `GenerateGo` output. Built-in fields use `StrictInteger`; named fields retain types backed by it; bounds are schema-owned. Non-0/0 local forms fail schema construction with no schema; global inline/anonymous forms are query-only; consumers reject. Choices/sequences, repetition/non-default, lists/unions, attributes, other integer-derived, nested/recursive/broader/anonymous unsupported/no output. Built-in/named Boolean/integer/decimal/string/token/NMTOKEN and inline string/token/NMTOKEN generate; only default direct-choice Boolean/integer/decimal refs are eligible.
