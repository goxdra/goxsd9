...
Model-group refs/extensions retain IDs/locations; consumers reject. Model-less bounded extensions require completed named empty-content bases; retain nil particles without synthetic content; bounded/representable inherited `##other`/lax wildcard facts. `anyAttribute`: default `##any`/strict; explicit `##any`/strict/`##other`/lax; locations. `xs:any`: `##any`/strict or `##other`/lax; `0/0` absent; nonzero rejected. `openContent mode="none"` supports globals/bounded extensions under Compatibility/Strict11; Strict10 mismatches. Other derivations unsupported; malformed=invalid.
